...
gates. Grouped by the defect each proves:
  ...
  structured evidence map -- validate_evidence resolves every gate's Assertion
    anchors against the ai/notes/ markers and enforces board-wide id uniqueness,
...
def check_evidence_map():
    ...
    # migrated gate's anchor resolves to a real <a id> marker in ai/notes/, and
...
def check_dirty_watch():
    ...
    nbr_line = " M ai/gates/board.py"           # a watched neighbor
    ...
    mutated = cfg_line.strip()               # "M ai/gates/board_config.json"
    ...
    owner_ok = (all(d in watched for d in run_board._EXECUTABLE_DIRS)
                ...
                and any(exclude == directory
                        or exclude.startswith(directory + "/")
                        for directory in run_board._EXECUTABLE_DIRS))
...
def _mf_body_undeclared_target(ctx):
    ...
    ctx.run_check("ai/gates/checks/board_selftest.py")   # a real check, auto-covered
...
def _mf_body_sentence_final_py(ctx):
    ...
    ctx.run_check("ai/gates/checks/board_selftest.py")   # auto-covered check
...
def check_runtime_loader_census():
    ...
    try:
        table = dict(saved)
        table.pop("ai/gates/checks/cmb_identity.py")
    ...
    # ai/gates/checks/gct_parity.py resolves against the importer's OWN directory,
    ...
    sib = run_board._module_to_repo_paths(
        "gsv_bitwise_drift", 0, "ai/gates/checks/gct_parity.py")
    report("25M-16 bare-sibling import resolves against the importer's dir",
           sib == ["ai/gates/checks/gsv_bitwise_drift.py"], str(sib))
    third = run_board._module_to_repo_paths(
        "torch", 0, "ai/gates/checks/gct_parity.py")
    ...
    report("25M-16 cobaya-adapter's closure includes the resolved sibling",
           "ai/gates/checks/gsv_bitwise_drift.py" in ca_closure,
...
def check_data_read_census():
    ...
    try:
        table = dict(saved)
        table.pop("ai/gates/checks/artifact_readback.py")
...
def check_manifest_riders():
    ...
    import tempfile, os
    empty = tempfile.mkdtemp(dir=str(run_board._REPO / "ai" / "gates"))
    try:
    ...
    real = str(run_board._REPO / "ai" / "gates" / "board.py")
...
def check_input_owner_resolution():
    ...
    (decoy_root / "ai" / "gates" / "configs").mkdir(parents=True)
    (decoy_root / "ai" / "gates" / "configs"
    ...
    bad = dict(cfg)
    bad["evaluate_yaml"] = "ai/gates/configs/does-not-exist.yaml"
    ca = [g for g in BOARD if g.id == "cobaya-adapter"][0]
...
def check_watch_tracked_drivers():
    ...
    (tmp / "ai" / "gates").mkdir(parents=True)
    (tmp / "ai" / "gates" / "board_config.json").write_text("{}\n")
    ...
    try:
        run_board._REPO = tmp
        run_board._EXECUTABLE_DIRS = ("emulator", "ai/gates")
        run_board._WATCH_EXCLUDE = "ai/gates/board_config.json"
        ...
        (tmp / "ai" / "gates" / "board_config.json").write_text('{"x": 1}\n')
        w, rc, off = offenders()
