...
# The region the artifacts this check PREDICTS THROUGH declare they stand for.
#
...
# data vector of the right shape that is wrong. The synthetic parameters here
# are p0 / p1 / p2 -- unnamed stand-ins with no physics behind them, drawn from
# a standard normal in every fixture below -- so the box a real emulator of this
# shape would have been drawn from is the four-sigma one, and it contains every
# row these legs train on and every point they ask about.
#
# Only the TRANSFER artifacts declare it: they are the ones the predictor is
# driven through. A frozen base is loaded as a base, never served as a
# prediction, so it declares no support at all -- and check_lifecycle proves
# that a base handed to a predictor anyway refuses to answer.
...
def check_lifecycle(device, tmp):
  ...
  # the frozen base is a base, never a prediction. It was generated by nobody
  # and is valid over nowhere, so its record declares no support and a predictor
  # pointed at it refuses the point rather than extrapolating over a region that
  # does not exist. The leg reads the WORDS of the refusal: float("n/a") raises
  # the same ValueError class every refusal here raises, so an arm that only
  # asked "did it raise?" would stay green with the law broken.
  base_predictor = EmulatorPredictor(str(base_root), device,
                                     dv_return="3x2pt")
  leaked = None
  try:
    base_predictor.predict(row)
  except ValueError as e:
    leaked = str(e)
  report("lifecycle: the frozen base declares no support and refuses to serve",
         leaked is not None and "declares no support" in leaked,
         "ValueError names the domain law" if leaked is not None
         else "a test double answered a question")
