...
two tiny dumps are ~400 serial CAMB calls at low accuracy):
  ...
  2b the Motloch & Hu NON-DIAGONAL terms (eq 6) end to end at smoke
     ...
     keys in the provenance (the weight fix, ai/notes/families-scalar-cmb.md).
...
def check_cov_nondiagonal(rootdir, rel_root, emul_dir, chains):
    ...
    # ai/notes/families-scalar-cmb.md): the derivative coordinate, the band
...
def main():
    ...
    # evidence set (ai/gates/board.py Gate id="cmb-smoke") is exactly these six
