...
| `cmb-covariance-publication` | Uses tiny NumPy archives on the CPU; it does not run CAMB or train an emulator. It checks that an existing CMB covariance survives unchanged, interrupted writes leave no public partial file, every saved member is reopened before publication, and a second file created during the calculation is not overwritten. |
