...
Glossary:
  ...
  evidence  = a gate's structured evidence map (Gate.evidence): the tuple
              ...
              before any test runs, that every anchor resolves in ai/notes/
...
class Assertion:
  ...
  anchor resolves to a real, explicitly-declared marker in ai/notes/, and
...
class Manifest:
  ...
  Arguments:
    code   = repo-relative production modules this gate's checks depend on
             ...
             and the ai/gates/checks/*.py scripts the gate body names (both added
...
class Gate:
  ...
  Arguments:
    ...
    spec_code = the key of this test's audit-history entry inside its home
              ...
              never printed or documented outside ai/notes/ (user ruling
    ...
    evidence= the structured evidence map: the acceptance legs this test
              ...
              anchor must resolve to a declared marker in ai/notes/, every id
...
def gate_diag(ctx):
  ...
  search over repository *.py files (ai/gates/, .git/ excluded) returns zero
  ...
  # exclude ai/gates/ (this harness's own gate_diag holds the literal
...
def gate_gb_c(ctx):
  ...
  """
  ctx.require_caps("torch", "cosmolike", "gpu")
  rc, out = ctx.run_check("ai/gates/checks/gb_c_berhu_reduce.py")
  if not ctx.dry:
    ctx.expect(
      label="berhu-loss leg 1 berhu/_reduce numerics + autograd continuity",
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/gb_c_berhu_reduce.py)")
  _golden_leg(ctx=ctx,
              gate_id="berhu-loss",
              yaml_name="cosmic_shear_train_emulator.yaml",
              grep_pattern="^(phase|epoch|best)",
...
def gate_gt_b(ctx):
  ...
  rc, out = ctx.run_check("ai/gates/checks/gt_b_triangle.py")
  if ctx.dry:
    return
  ctx.expect(label="triangle-shading four-window triangle shading check exit 0",
             ok=(rc == 0),
             detail="check exit code " + str(rc)
                    + " (ai/gates/checks/gt_b_triangle.py)")
...
def gate_gwd_c(ctx):
  ...
  """
  ctx.require_caps("torch", "gpu")
  rc, out = ctx.run_check("ai/gates/checks/gwd_census.py")
  if not ctx.dry:
...
def gate_gct_c(ctx):
  ...
  rc, out = ctx.run_check("ai/gates/checks/gct_parity.py")
  if not ctx.dry:
    ctx.expect(label="cobaya-adapter parity child completed",
               ok=(rc == 0),
               detail="check exit code " + str(rc)
                      + " (ai/gates/checks/gct_parity.py)")
...
def gate_ftw_a(ctx):
  ...
  rc, out = ctx.run_check("ai/gates/checks/finetune_identity.py")
  if not ctx.dry:
    ctx.expect(
      label="finetune-identity child completed",
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/finetune_identity.py)")
...
def gate_tpe_a(ctx):
  ...
  # the child's aggregate verdict and carries no aid of its own.
  rc, out = ctx.run_check("ai/gates/checks/transfer_identity.py")
  if not ctx.dry:
    ctx.expect(
      label="transfer-identity child completed",
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/transfer_identity.py)")
...
def gate_spe_a(ctx):
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/scalar_identity.py")
  if not ctx.dry:
    ctx.expect(
      label="scalar-identity round-trip + state + provides + all error legs",
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/scalar_identity.py)")
...
def gate_spe_b(ctx):
  ...
  rc, out = ctx.run_check("ai/gates/checks/scalar_smoke.py")
  if not ctx.dry:
    ctx.expect(
      label="scalar-smoke child completed",
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/scalar_smoke.py)")
...
def gate_cme_a(ctx):
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/cmb_identity.py")
  if not ctx.dry:
    ctx.expect(
...
def gate_cme_b(ctx):
  ...
  normalization fix, ai/notes/families-scalar-cmb.md). torch + cobaya + a
  compiled CAMB under $ROOTDIR; budget
  several minutes (~400 serial low-accuracy CAMB calls).

  HOW: the child (ai/gates/checks/cmb_smoke.py) folds its many human-readable
  ...
  (spec: ai/notes/families-scalar-cmb.md#cmb-smoke-evidence).
  """
  ctx.require_caps("torch", "cobaya")
  rc, out = ctx.run_check("ai/gates/checks/cmb_smoke.py")
  if not ctx.dry:
    ctx.expect(
      label="cmb-smoke generator + covariance + train + cobaya + diagnostics",
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/cmb_smoke.py)")
...
, superseding the old half-chunk form; the old form is the gate's
  ...
  HOW: the child (ai/gates/checks/bsn_identity.py) folds its many
  ...
  (spec: ai/notes/families-background-mps.md#bsn-identity-evidence).
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/bsn_identity.py")
  if not ctx.dry:
    ctx.expect(
      ...
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/bsn_identity.py)")
...
def gate_bsn_b(ctx):
  ...
  (spec: ai/notes/families-background-mps.md#bsn-smoke-evidence).
  """
  ctx.require_caps("torch", "cobaya")
  rc, out = ctx.run_check("ai/gates/checks/bsn_smoke.py")
  if not ctx.dry:
    ctx.expect(
...
def gate_mps_a(ctx):
  ...
  (spec: ai/notes/families-background-mps.md#mps-identity-evidence).
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/mps_identity.py")
  if not ctx.dry:
    ctx.expect(
      ...
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/mps_identity.py)")
...
def gate_mps_b(ctx):
  ...
  HOW: ai/gates/checks/mps_smoke.py runs four board-declared evidence legs,
  ...
  """
  ctx.require_caps("torch", "cobaya")
  rc, out = ctx.run_check("ai/gates/checks/mps_smoke.py")
  if not ctx.dry:
...
def gate_finite_contract(ctx):
 ...
 clauses).
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/finite_contract.py")
  if not ctx.dry:
    ...
    ctx.expect(
      ...
      ok=(rc == 0),
      detail=reason + " (ai/gates/checks/finite_contract.py)")
...
def gate_board_selftest(ctx):
  ...
  """
  rc, out = ctx.run_check("ai/gates/checks/board_selftest.py")
  if not ctx.dry:
    ctx.expect(
...
def gate_artifact_readback(ctx):
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/artifact_readback.py")
  if not ctx.dry:
    ctx.expect(
...
def gate_fixed_facts_schema(ctx):
  ...
  rc, out = ctx.run_check("ai/gates/checks/fixed_facts_schema.py")
  if ctx.dry:
    return
  ctx.expect(label="fixed-facts-schema child completed",
             ok=(rc == 0),
             detail="check exit code " + str(rc)
                    + " (ai/gates/checks/fixed_facts_schema.py)")
...
def gate_generator_seed(ctx):
  ...
  """
  rc, out = ctx.run_check("ai/gates/checks/generator_seed.py")
  if not ctx.dry:
    ctx.expect(
...
def gate_cli_strict(ctx):
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/cli_strict.py")
  if not ctx.dry:
    ctx.expect(
...
def gate_family_first(ctx):
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/family_first.py")
  if not ctx.dry:
    ctx.expect(
      ...
      ok=(rc == 0),
      detail="check exit code " + str(rc)
             + " (ai/gates/checks/family_first.py)")
...
def gate_stage_ram(ctx):
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/stage_ram.py")
  if not ctx.dry:
    ctx.expect(
...
def gate_diagnostics_domain(ctx):
  ...
  """
  ctx.require_caps("torch")
  rc, out = ctx.run_check("ai/gates/checks/diagnostics_domain.py")
  if not ctx.dry:
    ctx.expect(
