...
| `test_role_contract.py` | Reads the protected YAML contract with the same strict reader used by the watcher. The valid repository copy must say that a protected-policy draft receives one adversarial review. Duplicate, unknown, missing, or wrongly typed fields must stop, and changing either the 90-minute Implementer pause or the default 120-minute dispatch timeout in only one place must stop the watcher rather than silently changing policy. |
