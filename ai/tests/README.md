...
| `test_fixed_facts_record_matches_text.py` | Does a saved emulator refuse to load when one half of its scientific record was edited after writing while the generator's own text beside it was left alone? |
...
| `test_training_config_refusals.py` | Does a `train_args` file that asks for an anneal that cannot ramp, or a berhu knot equal to its cap, stop the run instead of training on a setting the file did not mean? |
...
| `mps_sigma8_child_checks.py` | Holds the numeric sigma-eight checks and the early matter-power request checks (`must_provide` refusing a wrong density pair, out-of-range redshift, or unservable wavenumber at setup) that `test_mps_sigma8_contract.py` launches in a child process, in the same `cobaya_minimal_stub/` arrangement as the dark-energy child checks. The launcher also runs the child once with a deliberately wrong expected value; that run must fail, proving a wrong sigma-eight cannot pass silently. |
...
| `test_relay_log_reservation.py` | Hands the dispatch-log reservation one frozen timestamp so two turns of the same role land in the same clock second on every run. Both reserved files must exist with their own complete contents, the collision must gain a two-digit suffix while the first name stays plain, different roles must share the second without any suffix, and a missing relay folder must be created instead of failing the first dispatch on a fresh checkout. |
