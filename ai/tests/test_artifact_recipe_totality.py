...
def _manifest_facts(**changes):
  """Return explicit external facts required by a compatibility manifest."""
  facts = {
    "parameter_geometry_cls": (
...
class ArtifactRecipeTotalityTests(unittest.TestCase):
  ...
  def test_manifest_builds_explicit_semantic_components(self):
    recipe = _recipe()
    manifest = artifact_recipe.build_compatibility_manifest(
      recipe, **_manifest_facts())
    self.assertEqual(manifest["schema"], 1)
    self.assertEqual(manifest["model"]["path"], recipe["cls"])
    self.assertEqual(manifest["trunk_activation"]["name"], "H")
    self.assertEqual(manifest["head_activation"]["mode"], "inherit")
    self.assertEqual(manifest["normalization"]["name"], "affine")
    self.assertEqual(manifest["composition_decoder"]["mode"], "plain")
    self.assertEqual(manifest["ia_coefficients"]["design"], "none")
    self.assertEqual(manifest["analytic_target"]["law"], "none")
    self.assertIn("semantic_id", manifest["output_geometry"])
    self.assertIs(
      artifact_recipe.validate_compatibility_manifest(
        manifest, recipe, **_manifest_facts()),
      manifest)

  def test_manifest_distinguishes_no_head_inherit_and_explicit_head(self):
    common = _manifest_facts()
    no_head = artifact_recipe.build_compatibility_manifest(
      _recipe("emulator.designs.plain.ResMLP"), **common)
    inherited = artifact_recipe.build_compatibility_manifest(
      _recipe(), **common)
    explicit_recipe = _recipe()
    explicit_recipe["kwargs"]["head_act"] = {
      "type": "tanh", "n_gates": 3}
    explicit = artifact_recipe.build_compatibility_manifest(
      explicit_recipe, **common)
    self.assertEqual(no_head["head_activation"], {"mode": "none"})
    self.assertEqual(inherited["head_activation"]["mode"], "inherit")
    self.assertEqual(explicit["head_activation"]["mode"], "explicit")
    self.assertEqual(explicit["head_activation"]["name"], "tanh")

  def test_manifest_missing_unknown_or_mismatched_fields_refuse(self):
    recipe = _recipe()
    kwargs = _manifest_facts()
    manifest = artifact_recipe.build_compatibility_manifest(recipe, **kwargs)
    missing = copy.deepcopy(manifest)
    del missing["model"]
    unknown = copy.deepcopy(manifest)
    unknown["approval"] = True
    mismatch = copy.deepcopy(manifest)
    mismatch["model"]["semantic_id"] = "model:forged:v1"
    for changed, message in (
        (missing, "missing.*model"),
        (unknown, "unknown.*approval"),
        (mismatch, "semantic_id")):
      with self.subTest(message=message), self.assertRaisesRegex(
          ValueError, message):
        artifact_recipe.validate_compatibility_manifest(
          changed, recipe, **kwargs)

  def test_manifest_refuses_unregistered_geometry_paths(self):
    with self.assertRaisesRegex(ValueError, "registered parameter geometry"):
      artifact_recipe.build_compatibility_manifest(
        _recipe(), parameter_geometry_cls="external.ParamGeometry",
        output_geometry_cls=(
          "emulator.geometries.output.DataVectorGeometry"),
        composition_mode="plain", ia_design="none", analytic_law="none")

  def test_manifest_requires_every_external_fact_without_fallbacks(self):
    facts = _manifest_facts()
    for key in ("composition_mode", "ia_design", "analytic_law"):
      incomplete = dict(facts)
      del incomplete[key]
      with self.subTest(missing=key), self.assertRaisesRegex(TypeError, key):
        artifact_recipe.build_compatibility_manifest(_recipe(), **incomplete)

  def test_manifest_registers_composition_ia_and_analytic_implementations(self):
    for mode in artifact_recipe.COMPOSITION_MODES:
      with self.subTest(composition_mode=mode):
        component = artifact_recipe.build_compatibility_manifest(
          _recipe(), **_manifest_facts(composition_mode=mode))[
            "composition_decoder"]
        self.assertEqual(component["mode"], mode)
        self.assertIn("semantic_id", component)

    for design in artifact_recipe.IA_DESIGNS:
      recipe = (_recipe() if design == "none" else _recipe(
        "emulator.designs.ia.TemplateMLP", ia=design))
      with self.subTest(ia_design=design):
        component = artifact_recipe.build_compatibility_manifest(
          recipe, **_manifest_facts(ia_design=design))["ia_coefficients"]
        self.assertEqual(component["design"], design)
        self.assertIn("semantic_id", component)

    law_geometries = {
      "none": "emulator.geometries.output.DataVectorGeometry",
      "as_exp2tau_ref": "emulator.geometries.cmb.CmbDiagonalGeometry",
      "log_offset": "emulator.geometries.grid.GridGeometry",
      "syren_linear": "emulator.geometries.grid2d.Grid2DGeometry",
      "syren_halofit": "emulator.geometries.grid2d.Grid2DGeometry",
    }
    for law in artifact_recipe.ANALYTIC_TARGET_LAWS:
      with self.subTest(analytic_law=law):
        component = artifact_recipe.build_compatibility_manifest(
          _recipe(), **_manifest_facts(
            output_geometry_cls=law_geometries[law],
            analytic_law=law))["analytic_target"]
        self.assertEqual(component["law"], law)
        self.assertIn("semantic_id", component)

  def test_manifest_refuses_unknown_or_inconsistent_external_facts(self):
    cases = (
      (_recipe(), _manifest_facts(composition_mode="hybrid"),
       "composition_mode"),
      (_recipe(), _manifest_facts(ia_design="nla"), "ia_design"),
      (_recipe(), _manifest_facts(analytic_law="mystery"), "analytic_law"),
      (_recipe(), _manifest_facts(analytic_law="log_offset"),
       "incompatible"),
    )
    for recipe, facts, message in cases:
      with self.subTest(message=message), self.assertRaisesRegex(
          ValueError, message):
        artifact_recipe.build_compatibility_manifest(recipe, **facts)

  def test_manifest_semantic_ids_for_external_facts_are_exact(self):
    recipe = _recipe()
    facts = _manifest_facts()
    manifest = artifact_recipe.build_compatibility_manifest(recipe, **facts)
    for key in ("composition_decoder", "ia_coefficients", "analytic_target"):
      changed = copy.deepcopy(manifest)
      changed[key]["semantic_id"] = "forged:v1"
      with self.subTest(component=key), self.assertRaisesRegex(
          ValueError, "semantic_id"):
        artifact_recipe.validate_compatibility_manifest(
          changed, recipe, **facts)
