"""Dark-energy coordinate checks for the matter-power adapter (child run).
...
reconstruct the names stored by the emulator.  These checks prove that the
...
entry is ai/tests/test_mps_dark_energy_adapter.py, which launches this
file in a child process whose PYTHONPATH names
ai/tests/cobaya_minimal_stub ahead of any installed package, exactly as
ai/tests/test_mps_sigma8_contract.py launches its own child.  The
adapter's cobaya imports therefore resolve to that on-disk stand-in,
selected before Python started: nothing here edits any process's import
table.
"""
...
if not getattr(cobaya, "COBAYA_MINIMAL_STUB", False):
  raise SystemExit(
    "mps_dark_energy_child_checks: the imported cobaya package is not the "
    "ai/tests/cobaya_minimal_stub stand-in. Run these checks through "
    "ai/tests/test_mps_dark_energy_adapter.py, which launches this file "
    "with the stand-in first on PYTHONPATH.")
...
class MPSDarkEnergyAdapterTests(unittest.TestCase):
  ...
  def test_dropped_sum_is_replaced_by_physical_requirements(self):
    ...
    law, fixed, needed = adapter_module._dark_energy_contract(
  ...
  def test_old_transformed_artifact_with_constant_w_record_refuses(self):
    ...
    with self.assertRaisesRegex(
      ...
      adapter_module._dark_energy_contract(
  ...
  def test_reconstruction_supplies_every_saved_coordinate_name(self):
    ...
    point = adapter_module._resolved_dark_energy_point(
  ...
  def _adapter(self):
    """Build the production calculate path around two observable fakes."""
  ...
  def test_nonzero_wa_reaches_predictors_and_syren_base(self):
    ...
    The pklin artifact predicts a zero surface, so the served linear
    spectrum equals the real Syren base exactly.  Comparing that served
    surface against the base evaluated directly at (w0, wa) = (-0.9, 0.2)
    — and requiring it to DIFFER from the wa = 0 base — proves the
    nonzero calculated wa reached the Syren evaluation, without
    replacing any live function.
    """
    ...
    expected_base = syren_base.base_pklin(
      k_mpc=adapter._k, z=adapter._z, As_1e9=2.1, ns=0.965, H0=67.0,
      Ob=0.049, Om=0.31, w0=-0.9, wa=0.2)
    base_without_wa = syren_base.base_pklin(
      k_mpc=adapter._k, z=adapter._z, As_1e9=2.1, ns=0.965, H0=67.0,
      Ob=0.049, Om=0.31, w0=-0.9, wa=0.0)
    served_linear = state[("Pk_grid", False, "delta_tot", "delta_tot")][2]
    np.testing.assert_array_equal(served_linear, expected_base)
    self.assertFalse(np.allclose(expected_base, base_without_wa))
