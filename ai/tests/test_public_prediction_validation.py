...
class AdapterPublicationTests(unittest.TestCase):
  ...
  def test_mps_refuses_a_broadcastable_syren_base_shape(self):
    module = _load_adapter("emul_mps.py", "shape_mps_adapter")
    adapter = self._mps_adapter(
      module, np.zeros((2, 2)), np.ones((2, 2)))
    adapter.p_lin.law = "syren_linear"
    params = {"As": 2.1e-9, "ns": 0.96, "H0": 70.0,
              "omegab": 0.05, "omegam": 0.3}
    state = {}
    with mock.patch.object(
        module.syren_base, "base_pklin", return_value=np.ones(2)):
      with self.assertRaisesRegex(ValueError, r"exact shape.*broadcasting"):
        adapter.calculate(state, want_derived=False, **params)
    self.assertEqual(state, {})

  def test_mps_refuses_broadcastable_predictor_surfaces(self):
    module = _load_adapter("emul_mps.py", "raw_shape_mps_adapter")
    cases = ((np.ones(2), np.ones((2, 2)), "linear-emulator output"),
             (np.ones((2, 1)), np.ones((2, 2)),
              "linear-emulator output"),
             (np.ones((2, 2)), np.ones(2), "boost-emulator output"),
             (np.ones((2, 2)), np.ones((2, 1)),
              "boost-emulator output"))
    for linear, boost, message in cases:
      with self.subTest(message=message):
        adapter = self._mps_adapter(module, linear, boost)
        state = {}
        with self.assertRaisesRegex(ValueError, message + ".*exact shape"):
          adapter.calculate(state, want_derived=False)
        self.assertEqual(state, {})

  def test_mps_checks_syren_parameters_and_bases_before_publication(self):
    module = _load_adapter("emul_mps.py", "syren_boundary_mps_adapter")
    good = {"As": 2.1e-9, "ns": 0.96, "H0": 70.0,
            "omegab": 0.05, "omegam": 0.3}
    for bad in (True, np.nan, np.inf, 10 ** 1000):
      with self.subTest(parameter=repr(bad)):
        adapter = self._mps_adapter(
          module, np.zeros((2, 2)), np.ones((2, 2)))
        adapter.p_lin.law = "syren_linear"
        params = dict(good, ns=bad)
        state = {}
        with mock.patch.object(module.syren_base, "base_pklin") as base:
          with self.assertRaisesRegex(ValueError, "Syren parameter 'ns'"):
            adapter.calculate(state, want_derived=False, **params)
        base.assert_not_called()
        self.assertEqual(state, {})

    for base_value, raises in ((np.full((2, 2), np.nan), False),
                               (np.ones(2), True)):
      with self.subTest(linear_base=repr(base_value)):
        adapter = self._mps_adapter(
          module, np.zeros((2, 2)), np.ones((2, 2)))
        adapter.p_lin.law = "syren_linear"
        state = {}
        with mock.patch.object(
            module.syren_base, "base_pklin", return_value=base_value):
          if raises:
            with self.assertRaisesRegex(ValueError, "Syren linear base"):
              adapter.calculate(state, want_derived=False, **good)
          else:
            self.assertFalse(
              adapter.calculate(state, want_derived=False, **good))
        self.assertEqual(state, {})
    ...
    for base_value, raises in ((np.full((2, 2), np.nan), False),
                               (np.ones(2), True)):
      with self.subTest(boost_base=repr(base_value)):
        adapter = self._mps_adapter(
          module, np.ones((2, 2)), np.zeros((2, 2)))
        adapter.p_boost.law = "syren_halofit"
        state = {}
        with mock.patch.object(
            module.syren_base, "base_boost", return_value=base_value):
          if raises:
            with self.assertRaisesRegex(
                ValueError, "Syren nonlinear-boost base"):
              adapter.calculate(state, want_derived=False, **good)
          else:
            self.assertFalse(
              adapter.calculate(state, want_derived=False, **good))
