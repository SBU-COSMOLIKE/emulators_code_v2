...
- subprocess: `ai/gates/checks/bsn_identity.py`; no nested subprocess, real
...
`ai/gates/checks/bsn_identity.py` and these durable records. Production code,
`ai/gates/board.py`, and `ai/gates/run_board.py` are unchanged. The independent
...
- subprocess: `ai/gates/checks/bsn_smoke.py`, which launches
...
- subprocess: `ai/gates/checks/mps_identity.py`; no nested subprocess, real
...
- files: temporary train/validation parameter tables and covariance files,
  ...
  `.h5`/`.emul` pairs, and a diagnostics PDF; the `$ROOTDIR` work directory
  is removed in `finally`.
- subprocess: `ai/gates/checks/mps_smoke.py`, which launches
...
Files (unit 36): emulator/background.py, ai/gates/checks/bsn_identity.py,
ai/gates/board.py, emulator/README.md, ai/notes/families-background-mps.md.
...
(ai/gates/checks/mps_smoke.py:394-398) is
...
`ai/gates/checks/mps_identity.py` child with Cocoa Torch 2.6.0.  The child
...
`mps-identity` child must then exit zero.  This is a `ai/gates/checks/` repair and
