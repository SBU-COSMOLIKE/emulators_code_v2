...
babysitting:
    ...
    git pull
    git checkout -- ai/gates/board_config.json    # drop local edits
    python ai/gates/run_board.py --check          # preflight only
    python ai/gates/run_board.py                  # the whole board
    git add ai/gates/logs && git commit && git push
...
- `ai/gates/run_board.py` — CLI + runner. Selectors: `--check`, `--list`,
  `--dry-run`, `--gate <id>`, `--tier`, `--from <id>`,
  `--force-rerun <id>` (flag BEFORE the id), `--debug`.
- `ai/gates/board.py` — the registry: one Gate per entry (id, tier,
...
- `ai/gates/checks/` — numeric check scripts (print acceptance VALUES,
  exit nonzero on any failed leg); `ai/gates/configs/` — smoke YAMLs;
  `ai/gates/logs/` — `<GATE_ID>.log` + `BOARD.md` + `board_status.json`.
...
CUDA). The full regression pass is `python ai/gates/run_board.py
...
ai/notes/ — not even in a header line (the user ruling in
...
Note-file pointers ("spec: ai/notes/x.md:lines") are the crosswalk. The
...
ai/gates/checks/ge_c_eval_bs.py is the one production exception to the
...
1. A deterministic executable-surface digest covers tracked
   ...
   the machine-local board_config.json override, ai/notes/ and
...
so that sweep must include ai/gates/checks/board_selftest.py, diagnostics_domain.py,
...
- 45M-85 (commit 2807d3f): all 84 internal 45M-* audit codes stripped from
  emulator/ + ai/gates/ Python (comments / docstrings / gate descriptions /
...
anchor must resolve to an explicit `<a id="...">` marker in `ai/notes/`, and
...
`ai/gates/checks/board_selftest.py`.
...
3. **External-script leg manifests.** Each `ai/gates/checks/*.py` prints a
...
Repair (commit pending, `ai/gates/run_board.py` + `ai/gates/checks/board_selftest.py`):
...
`ai/gates/`, and root drivers, but not `compute_data_vectors/`, `cobaya_theory/`,
...
- `ai/gates/README.md`: the "known gaps" paragraph claimed the dirty-tree watch
  covered only `emulator/` + `ai/gates/` + drivers and that an unknown selector
...
- `ai/gates/README.md` "## The 32 tests" and `emulator/README.md` "the acceptance
...
- `emulator/README.md`: the two `45M-12` public-prose leaks (the Simpson rule)
  replaced with the executed rule + a `ai/notes/families-background-mps.md`
...
declared-vs-executed honesty in `ai/gates/README.md`; the root/package README
...
4. Assertion ids are `<gate-id>.<plain-leg-name>`
   ...
   raw log must teach a reader, internal codes stay in ai/notes/.
...
Gate (`ai/gates/checks/stage_ram.py`, 21 legs, rebuilt): the byte-accounting legs
...
misparses that line only. `ai/gates/board_config.json` therefore escapes
...
`['M ai/gates/board_config.json']` as an offender (the false red), while
...
for the executed watch: `_WATCH_EXCLUDE = "ai/gates/board_config.json"`
...
leaks `M ai/gates/board_config.json` (false red); the config's bytes were
...
`_gate_code_digest` hashes only the gate body plus the `ai/gates/checks/*.py`
...
"stage-ram": {
  ...
  "manifest": {
    "code": [
      {"path": "ai/gates/run_board.py",          "sha256": "..."},
      {"path": "ai/gates/board.py",              "sha256": "..."},
      {"path": "ai/gates/checks/stage_ram.py",   "sha256": "..."},
...
- `cli-strict` and `geo-paths` have an `importlib`/`__import__` site in their
  own CHECK SCRIPT (`ai/gates/checks/cli_strict.py`, `ai/gates/checks/geo_paths.py`),
  ...
  `ai/gates/checks/` sites from the census (a check script is already a hashed
...
The FIRST gate (board-selftest), complete declaration:
  ...
  - code is empty: board-selftest names its own check script
    (ai/gates/checks/board_selftest.py) in its gate body, so that check is an
...
ai/gates/checks/mps_identity.py from a clean detached checkout on the
...
(Architect owns ai/notes/*.md, red team owns texnotes/, the Implementer
...
morning's stricter reading): the Implementer MAY edit ai/notes/*.md
...
2. Batch 5 geo-paths DONE (this landing); cli-strict deferred. geo-paths'
   ai/gates/checks/geo_paths.py:170 imported RETIRED legacy module names expecting
...
Factual verification (Architect): ai/gates/checks/__init__.py says "the
...
1. DIDACTICS-02 (navigation truth) lands FIRST — it is small,
   ...
   phases and dates moved to ai/notes/ where they belong.
...
ai/notes/user-didactics-and-python-voice.md BEFORE writing any of it.
...
ai/gates/README.md and locally where sharper; the three plain-language
...
ai/gates/ didactics landings alike.
...
DIDACTICS-02 ai/gates/checks/__init__.py landing — the file-by-file
...
ai/notes/red-team-audit-and-didactics-2026-07-13.md ("Durable DIDACTICS
...
filings live in ai/notes/red-team-audit-and-didactics-2026-07-13.md
...
- D1, navigation truth (DIDACTICS-02 + 58): the ai/gates/checks
...
history-pattern scan (emulator/ + cobaya_theory/ + ai/gates/);
...
ai/notes/user-didactics-and-python-voice.md BEFORE writing any prose.
...
The register (ai/notes/red-team-audit-and-didactics-2026-07-13.md,
...
- 87: ai/gates/README.md:3-7 "cosmic-shear ... runs every ... one raw
...
- 91, SHARPENED beyond the filing: ":34-35 byte-verbatim
  ...
  ai/gates/, syren/, emulator/, compute_data_vectors/ finds NO retained
...
2. FOUR FILE VISITS, one per README, queue-6 lane (doc-only,
   ...
   (87-89 MERGED with DIDACTICS-02's D1 landing — ai/gates/README.md
...
5. Acceptance additions for this wave: every repaired command is
   ...
   check covers ALL READMEs (clickable ../ai/notes/... paths); no
...
("... ai/gates/run_board.py."), and stops the latent .pyc/.pyx false
captures. Apply (?!\w) to run_board.py:1054 (the general census)
AND :975 (the ai/gates/checks-prefixed census — same .pyc class);
...
`ai/gates/checks/finite_contract.py`, which it reads to certify the compile-lane
...
`ai/gates/checks/` is placed on `sys.path`. `_module_to_repo_paths` rejects an
...
`ai/gates/checks/gsv_bitwise_drift.py`. `cobaya-adapter` therefore stays current
...
one `any(...)`. That is false for `ai/gates/checks/cli_strict.py`: its reviewed
...
`ai/gates/configs/cobaya-adapter-evaluate.yaml`: launched from the repository,
...
`ai/gates/checks/finite_contract.py:87` imports `CosmolikeChi2` and
...
`cobaya_theory/`, and `ai/gates/` found this as the only unresolved production
...
(`ai/gates/run_board.py:2004-2046`).  In `main`, the `--list` return at
...
```
python3 ai/gates/run_board.py --list --gate definitely-not-a-gate
python3 ai/gates/run_board.py --list --from definitely-not-a-gate
python3 ai/gates/run_board.py --list --force-rerun definitely-not-a-gate
```
...
`index >= start and not gate.optional` (`ai/gates/run_board.py:1720-1732`).
...
all code changes scoped to ai/gates/ + notes. Independent verification
...
```
python3 ai/gates/run_board.py --dry-run --gate family-first \
...
(`ai/gates/run_board.py:1852-1856`), and a successful child's status record stores
...
their prerequisite (`ai/gates/board.py:2079-2112`) and consume the saved emulator.
...
`_REPO.glob("*.py")` (`ai/gates/run_board.py:615-629`).  A deleted tracked driver
...
resolved member staled (`ai/notes/gates-and-board.md`, section C), and the phase-2
...
the ai/gates/README teaching (DIDACTICS-87's list) and the state
...
present verbatim; the landing is notes-only (11 files, all ai/notes/);
...
- 65c5bec (25M-26): completes 25M-20's persistence clause. Each child PASS
  ...
  DEFERRED to the D1 visit (per ruling): the ai/gates/README resume-state list
...
- 95/97/98/99 RATIFIED and FOLDED: no new landing slots — 95's
  ...
  ride the ROOT README visit (72-80); 99 rides the package/ai/gates/
...
DEFERRED to the D1 visit (per the 26 ruling): the ai/gates/README resume-state
...
- D1 NAVIGATION TRUTH's prose half now (the ai/gates/checks __init__
...
ai/gates/checks/*.py teaching until D4 + D5 land on any file both
would touch, and do NOT touch ai/gates/run_board.py / board.py /
...
1. THE UNIT-13 COVARIANCE PACKAGE: the 25M-08/11/12 trio
   ...
   covariance-specific); touching any SHARED ai/gates/checks file
...
check module (unit 13), ai/gates/checks/bsn_identity.py (unit 90), and
...
- ai/gates/checks teaching (the campaign's 03/04/06/07 batches):
...
sites across 40 gates and calls 26 `ai/gates/checks/*.py` scripts; 7 gates
...
- NAME: `<gate-id>.<plain-leg-name>` (ruling 4) — no spec-code prefix, lower
  ...
  (45M-*, DIDACTICS-*) stay in ai/notes/.
...
before the hardest gates use it:
  ...
  2. Pure in-process gates (no subprocess): reconciliation is only the expect
     ...
     recorder.
  3. Wrapper+child gates (board.py body + a `ai/gates/checks/*.py`): adds the
...
- RECONCILIATION. After a gate body returns without GateFailure, the runner
  ...
  `[evidence] <gate>: declared {aids} but executed {aids}`.
- CHECK-SCRIPT MANIFEST. Each `ai/gates/checks/*.py` prints, per leg, a machine-
...
the coordination point in Deliverable C. `ai/gates/checks/` stays exclusively the
...
- Check-script census CORRECTED: 25, not the proposal's 26. There
  are exactly 25 literal `ctx.run_check("ai/gates/checks/*.py")`
  sites, one per script, no constructed paths; ai/gates/checks/ holds
...
(root, emulator/, ai/gates/, syren/), 22 Python doc surfaces under
...
- compileall (emulator, cobaya_theory, syren, compute_data_vectors,
  ai/gates/checks) clean and board_selftest ALL PASS — MY runs, not
  the filing's.
...
- ai/gates/README.md's one row (bsn-identity) is an HONEST NARROWING
...
- COLLISION CHECK: no ai/gates/board.py, ai/gates/run_board.py, or
  ai/gates/checks/*.py edits — the Implementer's exclusive surfaces
...
1. QUEUE-2 NOTE-SIDE EVIDENCE BLOCKS. The red team drafts the
   ...
   anchors), and the ai/notes/ side collides with nothing the
...
lands the ##AID manifest increment and ai/gates/checks/ goes quiet;
...
PYTHONPATH=. python3 ai/gates/run_board.py --list rc 0 (the validator
...
- cbdd49e replaces the saved-artifact paragraph's development-ledger
  ...
  lands", plus a ai/notes/ pointer in the public README) with the
...
25M-36 CONFIRMED at ai/gates/checks/mps_identity.py:275-276: the
...
- The scalar-smoke home-note block is amended to NINE honestly
  named aids with transforms; ai/gates/board.py was correctly NOT
...
ai/gates/checks owner during fan-out): adopt the strengthened child,
...
the protocol chain, ai/gates/checks teaching at D4+D5) plus lane-3
...
new ai/gates/checks file, no existing file touched) implementing the
...
named-terms MemoryError refusal). REPAIR (ai/gates/checks/stage_ram.py, check_canonical_order):
...
had run the landing block). Scope exact: ai/gates/checks/
...
NEW standalone child ai/gates/checks/generator_ranges.py with NO
...
mps-identity hand-apply reached ai/gates/checks/mps_identity.py, so
...
THE TEST FILE (ai/tests/test_trf_token_width.py, 5 arms) is the
...
(06c9d8f) had rewritten ai/gates/checks/mps_identity.py (+127 lines,
...
- Target file: ai/gates/checks/scalar_smoke.py ONLY. ai/gates/board.py
...
- Validation gate: PYTHONPATH=. with the cocoa interpreter,
  ai/gates/checks/scalar_smoke.py runs green end-to-end and emits
...
wins on divergence), with cold-start routing in ai/notes/MEMORY.md
...
finite-contract (gate_finite_contract, child ai/gates/checks/
...
ai/tools/handoff_router.py — a clipboard relay for the three web
...
to chat only — the rewrite relays POINTERS to ai/notes/ entries
(notes-first) and archives captured blocks under ai/notes/relay/ as
...
2. DURABLE RECORDS MAKE IT LONG-TERM. Substantive work — designs,
   ...
   ai/notes/ before any chat message; chat is a short routing
...
4. THE OBJECTIVITY ANCHOR. Validation gates are executed by the
   machine (ai/gates/run_board.py and the check scripts), and the
   relay tooling runs them locally and archives raw logs — an
   agent's pass/fail claim is never accepted without the command
   output behind it.

5. THE TOOLS, one sentence each: ai/tools/handoff_router.py (a
   clipboard relay between the three sessions, plus a --status
   sweep that reports the loop's state from git and the notes)
   and ai/tools/mailbox_daemon.py (a file mailbox under
   ai/notes/mailbox/ dispatched headlessly, so the loop runs without
...
- "Durable records make it long-term" — substantive work written under
  ai/notes/ before any chat message; chat is a pointer; the note wins on
...
- "The objectivity anchor" — gates run by the machine
  (ai/gates/run_board.py + the check scripts), relay tooling runs them
  locally and keeps raw logs, no pass/fail claim accepted without the
  command output. [contract 4]
- "The tools" — one sentence each for ai/tools/handoff_router.py (relay +
  --status loop-state sweep from git and notes) and
  ai/tools/mailbox_daemon.py (file mailbox under ai/notes/mailbox/, dispatched
...
ai/tools/handoff_router.py present, --status arg present (argparse line
288); ai/tools/mailbox_daemon.py present; ai/gates/run_board.py present;
ai/gates/checks/ holds the individual check scripts; ai/notes/mailbox/
...
limited to `ai/gates/checks/scalar_smoke.py`.  Its existing checks, calibrated
...
`ai/notes/families-scalar-cmb.md` and prints one terminal verdict for each group.
`ai/gates/board.py` remains untouched for the Implementer's declared-versus-
...
- `ai/tools/handoff_router.py` exists; `--status` is a real argparse
  flag (line 288).
- "the relay tooling runs them locally and keeps the raw logs" is
  ...
  just asserted.
- `ai/tools/mailbox_daemon.py` exists; dispatches headlessly from
  ai/notes/mailbox/ and writes a per-dispatch log (lines 161-174).
- `ai/gates/run_board.py` and the `ai/gates/checks/` scripts exist.
...
anchor" pins the board to `ai/gates/run_board.py`, and "board" has
...
- python ai/tools/handoff_router.py --status  (what the sweep
...
- python ai/tools/mailbox_daemon.py --send opus --unit "..."  (one
  ...
  carry the work).
- python ai/tools/mailbox_daemon.py --watch  (leave it running; each
  dispatch prints an rc line when the agent's turn completes;
  interrupting the terminal kills the running turn).
- python ai/tools/mailbox_daemon.py --ping opus  (the transport
...
- right box: the red-team session (attack and probe: bugs, weak
  ...
  self-applied.
- bottom fork: pass -> milestone recorded in ai/notes/; fail ->
  delta re-handoff.
...
- CURRENT-STATE CORRECTIONS both TRUE at the source: the refine
  ...
  stale-dependency resume state is live in ai/gates/run_board.py
  (:2102-:2130, :2381, :2657).
- MY UNSCRIPTED PROBE: the guide's eight aggregate-only gates
  equal the machinery exactly — 8 of 40 registered gates in
  ai/gates/board.py carry no evidence= tuple, and the sets match
...
summary: ai/notes/mailbox/0013-to-user.md.
...
so the evidence blocks I wrote in `ai/notes/artifacts-inference-warmstart.md`,
...
- `ai/gates/checks/board_selftest.py`: **176 PASS / 0 FAIL** (ALL PASS).
...
files:   ai/gates/board.py, ai/gates/checks/finetune_identity.py,
         ai/gates/checks/transfer_identity.py, ai/gates/checks/gsv_bitwise_drift.py,
         ai/gates/checks/gct_parity.py, ai/notes/artifacts-inference-warmstart.md,
...
carries ONE file I did not author: `ai/tools/mailbox_daemon.py` (+5/-2, the
...
- `ai/gates/board.py --list` rc 0 (all anchors resolve, no collisions).
- `ai/gates/checks/board_selftest.py` rc 0, zero `[FAIL]` markers,
  final line "board-selftest: ALL PASS".
- `ai/gates/checks/finetune_identity.py`: 7/7 `##AID ... PASS`.
- `ai/gates/checks/transfer_identity.py`: 8/8 emitted, 7 PASS +
...
Architect's live ai/tools/mailbox_daemon.py preamble fix (+5/-2), swept by
...
4. One runnable, fenced example (the README rule: concepts ship with a
   real snippet):
       python ai/tools/mailbox_daemon.py --send opus --unit "..."
       python ai/tools/mailbox_daemon.py --send sol  --unit "..."
       python ai/tools/mailbox_daemon.py --watch
...
on THIS SAME unit: ai/notes/mailbox/0012-to-opus.md carries the identical
...
then the turn's exit status and the ai/notes/relay/ log path; a dispatched
...
ai/tools/handoff_router.py; --dry-run/--once/--watch/--send/--ping/--unit in
ai/tools/mailbox_daemon.py). The --status output in the passage is a
...
recorded in ai/notes/" and "fail: delta re-handoff", a loop-repeats return
...
CHECKS RUN THIS TURN (raw):
  ...
  - python ai/tools/handoff_router.py --status: rc 0, real output (used to
...
Outbound routing summary: ai/notes/mailbox/0024-to-fable.md (0022 and 0023
...
ai/gates/checks/finite_contract.py + the register and home-note readback.
...
- Gate: python3 ai/tools/mailbox_daemon.py --dry-run runs correctly from
...
2. One worktree is the COORDINATION worktree: the one whose
   ai/notes/mailbox the daemon watches — the daemon derives every path
...
MILESTONE: one commit (README.md + ai/tools/mailbox_daemon.py), handoff
...
`Gate(id="scalar-smoke")` in `ai/gates/board.py` still carried no `evidence=`.
...
- `py_compile` on `ai/gates/board.py` + `ai/gates/checks/scalar_smoke.py`: clean.
...
- `ai/gates/checks/board_selftest.py`: **176 PASS / 0 FAIL**, final line
...
- **LIVE GREEN — the child ran here, no workstation debt.** Unlike the four
  ...
  `ai/gates/checks/scalar_smoke.py` on that interpreter: **rc 0, 9 `##AID`
...
branch:  claude/amazing-keller-e798b6
commit:  <this increment's sha — see the handoff>
files:   ai/gates/board.py, ai/notes/gates-and-board.md
...
cocoa interpreter: the child ai/gates/checks/scalar_smoke.py emits exactly
...
## RED-TEAM UNIT: ai/tools/ relay scripts review (Fable, 2026-07-14, user-directed)

USER instruction (mailbox 0027): red-team review of
ai/tools/handoff_router.py and ai/tools/mailbox_daemon.py. Handoff cut to
...
audit's sequencing consequence. Files: `ai/gates/checks/finite_contract.py`
(the emission wrapper), `ai/gates/board.py` (the evidence tuple + wrapper
prose), `ai/notes/training-stack.md` (the fourteen anchors + the gate's
evidence block).
...
1. **Wrapper label narrowed**, `ai/gates/board.py` `gate_finite_contract`:
...
- `PYTHONPATH=. <cocoa python> ai/gates/checks/finite_contract.py`: **rc 2**,
...
- `run_board.py --list`: **rc 0** — `validate_evidence` ran, so all fourteen
  ...
  aid collides board-wide.
- `ai/gates/checks/board_selftest.py`: **176 PASS / 0 FAIL**, `board-selftest:
...
branch:  claude/amazing-keller-e798b6
commit:  <this increment's sha — see the handoff>
files:   ai/gates/board.py, ai/gates/checks/finite_contract.py,
         ai/notes/training-stack.md, ai/notes/gates-and-board.md
...
GATE, leg by leg, with what actually ran.
  ...
  3. THE EXAMPLE'S COMMANDS RUN AS WRITTEN: GREEN on the flags, by
     `python3 ai/tools/mailbox_daemon.py --help` (rc 0): --send AGENT,
  ...
  4. "verify --send with a throwaway body against --dry-run": NOT
     ...
     ai/tools/ change this unit was not authorized to make.
...
a SECOND, separate commit (ai/notes/gates-and-board.md only), because
...
ai/notes/conventions-and-workflow.md modification present in the worktree
...
### CODE HALF - ai/tools/mailbox_daemon.py
...
- `WORKTREE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))` -
  the file lives at `<worktree>/ai/tools/mailbox_daemon.py`, so the directory
  above `ai/tools/` is the worktree root. MAILBOX, DONE and RELAY_DIR hang off it
  unchanged.
...
the repo-root copy would watch `<repo>/ai/notes/mailbox`, a different mailbox. The
sentence now says both live in `ai/tools/` and are run from a checkout, and that
which checkout you run the daemon from is itself meaningful, pointing at the new
subsection.

### GATE RESULTS (this session, raw)

- `python3 -m py_compile ai/tools/mailbox_daemon.py`: clean.
...
- **A fresh clone runs unedited.** An UNEDITED copy of the shipped file placed
  at `/tmp/fake-clone-probe/.claude/worktrees/some-other-name/ai/tools/` and
  ...
  `/tmp/fake-clone-probe/.claude/worktrees/some-other-name/ai/notes/mailbox/0001-to-opus.md`,
...
- **The 0022 unrunnable leg, now run against the LIVE mailbox:**
  ...
  `[dry-run] would queue .../ai/notes/mailbox/0035-to-opus.md`, and the live mailbox
...
- **Snippet fidelity:** the README's `AGENT_COMMANDS` block compared
  programmatically against the shipped block in `ai/tools/mailbox_daemon.py` -
  19 lines vs 19 lines, `VERBATIM MATCH: True`.
- **Board untouched:** no file under `ai/gates/` is in this diff.
...
2. **OPERATIONAL, act on this first:** this commit edits `ai/tools/mailbox_daemon.py`,
...
emission per ai/notes/gates-and-board.md, section 'RULING: finite-contract
...
to teach. They now carry one running story a newcomer reads at a glance:
  ...
  attack request (to sol, README:3651)
    "Try to break the new --version flag, as described in
     ai/notes/version-flag-attack.md."
...
- Copy-paste-run: both new commands executed verbatim under `--dry-run`.
  ...
  mailbox: "[dry-run] would queue .../ai/notes/mailbox/0037-to-opus.md"
...
the examples (`ai/notes/version-flag.md`, `ai/notes/version-flag-attack.md`)
...
the Implementer lane (ai/tools/mailbox_daemon.py, printed as a tripwire
...
(stable manifest naming) in ai/notes/training-stack.md plus their
25M-05/06 amendments. Also fired: TEX-PROSE-04+05+06 (register-defined
in ai/notes/red-team-audit-and-didactics-2026-07-13.md), one unit, three
subagent fan-outs.
...
the number lives in one place, BACKUP_THRESHOLD in ai/tools/mailbox_daemon.py;
...
no ` -- `); no all-caps emphasis; backticked path ai/tools/mailbox_daemon.py
exists. (2) Number-vs-code: README "ten or more units" vs the shipped
`BACKUP_THRESHOLD = 10` (ai/tools/mailbox_daemon.py:139). No gate surface was
...
- `.ranges` is then read by nothing in `emulator/` or `cobaya_theory/`. Its only
  ...
  (`ai/gates/checks/generator_ranges.py`, which AST-pins exactly one production
...
`ai/gates/checks/gsv_bitwise_drift.py:370-383` forges `schema_version = 1` and
...
(`ai/gates/checks/generator_ranges.py:100-104`) and cannot carry non-range facts;
...
- `ai/gates/checks/board_selftest.py`: **182 PASS / 0 FAIL, "board-selftest: ALL
  PASS"** (176 before, +6 from the new arm).
- `ai/gates/run_board.py --list`: rc 0 (all anchors resolve, no collisions).
...
```
branch:  claude/amazing-keller-e798b6
files:   ai/gates/checks/bsn_smoke.py, ai/gates/checks/mps_smoke.py,
         ai/gates/checks/cmb_smoke.py, ai/gates/checks/board_selftest.py,
         ai/notes/gates-and-board.md
...
as-is under ai/gates/checks/ with their names carrying the redteam_
...
ROUTING: 41-REPAIR and 53-REPAIR enter ai/notes/backlog.md as execution
...
total-demand count (mailbox + ai/notes/backlog.md) the code now ships.
...
ai/notes/backlog.md, defined in place as the architect's ledger of jobs still owed.
...
reads off ai/notes/backlog.md today.
...
1. AGENT_COMMANDS quote vs source, byte for byte: True (1691 bytes, 32 lines,
   both sides), by extracting the block from ai/tools/mailbox_daemon.py and from
...
- Census re-read by me, not taken from the handoff: the three `LEG_AIDS`
  lists match the `Assertion(...)` tuples in `ai/gates/board.py` name-for-name
...
- File surface re-verified: b193849 touches README.md only (+107/-16);
  f1a3f16 touches ai/notes/gates-and-board.md only. No gate surface moved. The
...
- Gate 1 REPRODUCED: the `AGENT_COMMANDS` block extracted from the README
  fence and from `ai/tools/mailbox_daemon.py` compares byte-identical — 1691
...
- Gate 2 REPRODUCED: the shipped `report_demand()` was CALLED (module import,
  ...
  commit (`git show b193849:ai/notes/backlog.md` counts exactly 22 `- OPEN`
...
Suggested squash message (edit at will):
    ...
    queued mailbox messages plus the open lines of the ai/notes/backlog.md
...
- **`ai/gates/checks/fixed_facts_schema.py` (NEW)** + `ai/gates/board.py` +
  `ai/notes/artifacts-inference-warmstart.md` anchors — see the gate section.
- **`ai/gates/checks/gsv_bitwise_drift.py`** — the AUTHORIZED version arm: forges
...
`ai/notes/red-team-audit-and-didactics-2026-07-13.md` on the child branch,
...
- Working-tree state matched the handoff exactly before the commit: two
  unstaged files, numstat 102/14 (`ai/gates/checks/scalar_smoke.py`) + 124/0
  (the durable-record note), tip `77a1572`, base `b74d81b`, `ai/gates/board.py`
...
- **Static census re-derived by AST**, not taken from the handoff:
  `LEG_AIDS` = 9; `main:ai/gates/board.py` scalar-smoke assertions = 9 with
...
"scalar-smoke skipped-leg doctrine" leaves `ai/notes/backlog.md` this turn
...
files:  ai/gates/checks/scalar_smoke.py
...
merge:  cd /Users/vivianmiranda/data/COCOA/june2026/emulators_code_v2
        ...
        # EXPECTED CONFLICT in ai/notes/red-team-audit-and-didactics-2026-07-13.md:
...
Neither `ai/notes/backlog.md` nor the tools-review defect register carried a
...
- **`ai/gates/checks/generator_ranges.py`** — the acceptance bar: the two 25M-06
...
- **`ai/gates/checks/gct_parity.py`** — the ONE bare-positional caller in the repo
...
- **`ai/gates/checks/fixed_facts_schema.py`** — 34 -> **79 PASS**, 7 -> **12 aids**.
...
4. **`emul_cosmic_shear` has no refusal arm anywhere** in `ai/gates/` or `ai/tests/`.
...
`ai/notes/mailbox-daemon-incident-2026-07-14.md`, "Live reproduction 4."
...
ai/gates/board.py                               |  21 +-
ai/gates/checks/bsn_identity.py                 |  95 ++-
ai/gates/checks/cmb_identity.py                 |  69 +-
ai/gates/checks/fixed_facts_schema.py           | 996 +++++++++++++++++++++++++++-
ai/gates/checks/gct_parity.py                   |  15 +-
ai/gates/checks/generator_ranges.py             | 203 +++++-
ai/gates/checks/mps_identity.py                 |  56 +-
ai/gates/checks/scalar_identity.py              |  62 +-
ai/notes/MEMORY.md                              |  21 +
ai/notes/artifacts-inference-warmstart.md       |  61 ++
ai/notes/gates-and-board.md                     | 379 +++++++++++
ai/notes/mailbox-daemon-incident-2026-07-14.md  | 125 ++++
...
`emul_scalars`). Unit 82 is ready to leave `ai/notes/backlog.md` on a GO; units
...
Every hunk of every `ai/gates/` diff was read. `board.py`: purely additive (5 aids
...
- **F1 — the amendment's gate leg was never armed.** The :10954 amendment
  ...
  refuses"; nothing in `ai/gates/` or `ai/tests/` exercises `_as_row`'s refusals
...
Commit A carries the 11 code files + `ai/notes/artifacts-inference-warmstart.md`
...
- **The one thing that is NOT done: the audit turn's outbound never left.**
  ...
  0140-to-opus," and two `ai/notes/backlog.md` OPEN lines (units 84/85) repeat
...
- **Repair executed this turn:** `ai/notes/mailbox/0140-to-opus.md` written from
...
- **Nothing else was touched.** No code, no gate re-run, no landing
  ...
  `ai/notes/mailbox-daemon-incident-2026-07-14.md` and rides the tools-review
...
- **Recorded:** an addendum under Live reproduction 5 in
  `ai/notes/mailbox-daemon-incident-2026-07-14.md` (the clean-turn archival
...
`ai/notes/training-stack.md`.  The user explicitly assigned Sol as second
...
ai/gates/checks/redteam_unit41_policy_witness.py` (rc 0 meant the negative review
...
`ai/gates/checks/redteam_unit41_policy_witness.py`.
...
git add ai/gates/checks/redteam_unit41_policy_witness.py
git add ai/notes/gates-and-board.md ai/notes/training-stack.md ai/notes/backlog.md
git add ai/notes/MEMORY.md ai/notes/mailbox/0147-to-fable.md
...
- Both "- OPEN" lines leave ai/notes/backlog.md as CANCELLED, not as GO:
...
`ai/notes/gates-and-board.md` entry "Unit 53-REPAIR second-Implementer return"
...
7. **The cs rider — a NEW BOARD GATE.** `emul_cosmic_shear` is exercised by NO
   ...
   `ai/gates/checks/cs_adapter_identity.py` is the missing fifth — board gate
...
- **Pre-existing at my session start (unit 41-REPAIR, second Implementer):**
  ...
  `ai/gates/checks/redteam_unit41_policy_witness.py`, `ai/notes/MEMORY.md`,
  `ai/notes/training-stack.md`.
- **Arrived DURING my turn, from a concurrent lane** (the unit-13 covariance
  readback and the unit 53-REPAIR return): `ai/notes/backlog.md`, the unit-53 section
  appended to this note, `ai/notes/red-team-audit-and-didactics-2026-07-13.md`, and
  most of the new lines in `ai/notes/families-scalar-cmb.md` (my part of that file is
...
`cobaya_theory/emul_*.py`, `ai/gates/board.py`, `ai/gates/run_board.py`,
`ai/gates/checks/{fixed_facts_schema,scalar_identity,bsn_identity,cmb_identity,mps_identity,transfer_identity}.py`,
the new `ai/gates/checks/cs_adapter_identity.py`, the anchor blocks in
`ai/notes/artifacts-inference-warmstart.md`, `ai/notes/families-background-mps.md` and
`ai/notes/families-scalar-cmb.md`, and this section.
...
## AI-loop documentation: the section moved out to `ai/README.md`, and the daemon's options got a passage (Implementer, 2026-07-14, mailbox dispatch riding 0045)
...
was still owed when the second arrived: `ai/README.md` did not exist and
README.md still carried the whole AI section as its section 24.

### What changed

`ai/README.md` (new, 602 lines) is now the loop's own document. It carries
...
sentences pointing at `ai/README.md`. README.md went from 3,907 to 3,490
lines.

The new section, "The command line options", is anchored on `--help`: it tells
the reader that `python ai/tools/mailbox_daemon.py --help` always prints the
...
overflow language; every occurrence of "default" in `ai/README.md` is the
...
1. **The merge-authority sentence changed, on the blueprint's own order.** The
   ...
   blueprint is the contract, so `ai/README.md` now says the architect is
...
3. **Files beyond the two named.** The commit also carries this note section
   and its `ai/notes/MEMORY.md` index line, per the notes ritual and role rule 7
...
- **`--fix-only` is documented nowhere, deliberately.** Backlog line 42 says
  the flag will be "document[ed] in --help and the `ai/README.md` options
...
- **GREEN-after, reproduced exactly:** `ai/gates/checks/redteam_unit41_policy_witness.py`
...
- **Production substance: PASS.** Both authorized halves implemented as
  ...
  `ai/gates/board.py` has no unit41 reference).
...
`ai/gates/checks/redteam_unit41_policy_witness.py`, with no production change.
...
# no output
py_compile ai/gates/checks/redteam_unit41_policy_witness.py: PASS
AST comprehension/lambda/90-column scan: PASS
git diff --check ai/gates/checks/redteam_unit41_policy_witness.py: PASS
```
...
```text
git add ai/gates/checks/redteam_unit41_policy_witness.py
git add ai/notes/gates-and-board.md ai/notes/backlog.md
git add ai/notes/mailbox/0159-to-fable.md
...
4. **The base witness digest is PINNED for the gate-integrity diff:**
   ...
   for `ai/gates/checks/redteam_unit53_manifest_witness.py` at `14b416c` —
   byte-identical at main `f347b8f`, at branch tip `c224a79`, and in this
   working tree. At fetch, the branch's whole `ai/gates/` diff must reduce to
...
`ai/README.md` and give the daemon's command line options their own
...
- The fenced `--help` block in ai/README.md (lines 185–227) is
  byte-identical to live `python3 ai/tools/mailbox_daemon.py --help` output
  (whitespace-normalized diff empty; checked programmatically).
- The fenced `build_agent_commands()` block is present VERBATIM in
  ai/tools/mailbox_daemon.py (substring check, exact).
...
One defect, repaired inline as an audit rider: ai/README.md:12 carried
...
ai/notes/mailbox, which no watch polls, so messages 0002-0012 sat
...
- 0003 + 0005 + 0006 (section-24 editorial: verbosity, drop the cryptic
  ...
  ai/README.md (the section has moved) and dispatched as 0156-to-opus.
...
All eleven files are moved to the main checkout's ai/notes/mailbox/done/ so
...
`git diff` over `ai/gates/` was read deletion-by-deletion: every removed line is
...
their authorizing ruling and all eight check out. One diff hunk in `ai/gates/`
...
unit 41-REPAIR witness delta (`ai/gates/checks/redteam_unit41_policy_witness.py`,
Sol's 0159 return — it awaits its OWN audit next turn) and the concurrent-lane
edit to `ai/notes/mailbox-daemon-incident-2026-07-14.md`. Committed here: the
...
Sol's delta to ai/gates/checks/redteam_unit41_policy_witness.py (+22/-11,
...
1. **Daemon repro at the tip: 8/8 ARM PASS** (`ai/tests/
...
2. **Router repro at the tip: ALL SCRATCH ROUTER REPRODUCTIONS PASS**
   (`ai/tests/tools_handoff_router_repro.py`): cwd anchoring (repo-derived
...
3. **Red-before, executed not quoted**: the SAME tip test files were run
   against the EXACT base (`git show 204748e:ai/tools/...` into a second
...
`ai/gates/checks/finite_contract.py`, a single string — the non-green
...
(`ai/tests/finite_contract_cuda_wording_repro.py`) passes at the tip and
...
`ai/tools/mailbox_daemon.py` diverged: the tip hardens the 614-line base
...
directed Codex to take over the pending `ai/notes/backlog.md` work, ignore the
...
cocoa-torch python: ai/gates/checks/stage_ram.py           rc 0  ALL PASS
...
cocoa python: ai/gates/checks/fixed_facts_schema.py                  rc 0  88 PASS / 0 FAIL; 13 aids
...
```text
python3 ai/tests/tools_handoff_router_repro.py          rc 0  7/7 arms PASS
python3 ai/tests/finite_contract_cuda_wording_repro.py  rc 0  PASS
python3 ai/tools/handoff_router.py --help               rc 0  only redteam/second-implementer
...
does not touch `ai/tools/mailbox_daemon.py` or claim any of its later riders.
...
```text
python3 ai/tests/tools_mailbox_daemon_redteam_repro.py  rc 0  8/8 arms PASS
...
unconditional clauses in `.claude/OPUS_ROLE.md`, the `ai/notes/MEMORY.md` header,
and the canonical sentence in `ai/notes/conventions-and-workflow.md`. The already
...
```text
python3 -B ai/tests/test_mailbox_conditional_preamble.py  rc 0  4/4 tests green
python3 -B ai/tests/tools_mailbox_daemon_redteam_repro.py rc 0  8/8 arms PASS
...
```text
python3 -B ai/tests/tools_mailbox_daemon_output_style_repro.py  rc 0  8/8 checks PASS
python3 -B ai/tests/tools_mailbox_daemon_redteam_repro.py       rc 0  8/8 arms PASS
...
PYTHONPATH=. Cocoa/.local/bin/python ai/gates/checks/mps_identity.py
                                                              rc 0  7/7 AIDs green
...
```text
python3 ai/tests/tools_mailbox_daemon_staleness_repro.py  rc 0  18/18 runtime arms PASS
source mutations                                       rc 0  9/9 killed
python3 ai/tests/tools_mailbox_daemon_redteam_repro.py    rc 0  8/8 arms PASS
python3 ai/tests/tools_mailbox_daemon_output_style_repro.py rc 0  8/8 checks PASS
python3 -m unittest ai/tests/test_mailbox_conditional_preamble.py rc 0  4/4 green
py_compile                                             rc 0
git diff --check                                       rc 0
```
...
```text
python3 ai/tests/tools_mailbox_daemon_dead_mailbox_repro.py rc 0  9/9 runtime arms PASS
source mutations                                        rc 0  7/7 killed
python3 ai/tests/tools_mailbox_daemon_redteam_repro.py     rc 0  8/8 arms PASS
python3 ai/tests/tools_mailbox_daemon_output_style_repro.py rc 0 8/8 checks PASS
python3 -m unittest ai/tests/test_mailbox_conditional_preamble.py rc 0 4/4 green
python3 ai/tests/tools_mailbox_daemon_staleness_repro.py   rc 0 18/18 + 9/9 mutations
py_compile                                              rc 0
git diff --check                                        rc 0
```
...
```text
python3 ai/tests/tools_mailbox_daemon_fix_only_repro.py      rc 0  14/14 runtime arms PASS
focused source mutations                                  rc 0  20/20 killed
python3 ai/tests/tools_mailbox_daemon_dead_mailbox_repro.py rc 0  9/9 + 7/7 mutations
python3 ai/tests/tools_mailbox_daemon_redteam_repro.py      rc 0  8/8 arms PASS
python3 ai/tests/tools_mailbox_daemon_output_style_repro.py rc 0  8/8 checks PASS
python3 -m unittest ai/tests/test_mailbox_conditional_preamble.py rc 0  4/4 green
python3 ai/tests/tools_mailbox_daemon_staleness_repro.py    rc 0  18/18 + 9/9 mutations
...
```text
python3 -B ai/tests/tools_mailbox_daemon_rendezvous_repro.py rc 0  9/9 runtime; 7/7 mutations killed
...
python3 -m py_compile ai/tools/mailbox_daemon.py ai/tests/tools_mailbox_daemon_rendezvous_repro.py ai/tests/tools_mailbox_daemon_staleness_repro.py rc 0
git diff --check                                           rc 0
```
...
```text
python ai/tools/mailbox_daemon.py --watch --architect-model opus --implementer-model sonnet
```
...
python3 -B ai/tests/tools_mailbox_daemon_output_style_repro.py rc 0  8/8 checks
python3 -B -m unittest tests.test_mailbox_conditional_preamble rc 0  4/4
python3 -B -m py_compile ai/tools/mailbox_daemon.py ai/tests/tools_mailbox_daemon_role_models_repro.py rc 0
git diff --check                                            rc 0
```
...
```text
PYTHONPATH=. /Users/vivianmiranda/miniforge/envs/cosmology/bin/python -B ai/gates/checks/redteam_unit53_manifest_witness.py  rc 0  ALL PASS
python3 -B -m py_compile emulator/studies/*.py cosmic_shear_tune_emulator.py ai/gates/checks/redteam_unit53_manifest_witness.py  rc 0
