...
survives in git history (`git log --follow ai/notes/<old-name>.md`); the
...
Implementer and the Red Team is written under `ai/notes/` before a chat relay is
...
same rule governs `ai/notes/mailbox/` dispatches: a mailbox file is a routing
...
`ai/notes/`. The explicit binding TERMINAL/no-reply exception and ambiguity rule
...
- [Gates + the board](gates-and-board.md) — the harness, identity/smoke
  ...
  and run history. `ai/gates/board.py` plus `python3 ai/gates/run_board.py --list`
...
- [AI-loop docs moved to ai/README.md + the daemon options passage](gates-and-board.md#ai-loop-documentation-the-section-moved-out-to-notesreadmemd-and-the-daemons-options-got-a-passage-implementer-2026-07-14-mailbox-dispatch-riding-0045) — the 0045 restructure and the options directive land in one commit: README.md section 24 is now a three-sentence pointer (the `#24-ai-usage` anchor is kept) and `ai/README.md` is the loop's own document, with a new `--help`-anchored options section covering all twelve flags (the fenced help block is machine-diffed byte-exact against the real command, and compaction plus the two separate context keys are explained from scratch). Three things for the Architect: the merge-authority sentence now says the architect merges (the 0045 fifth deliverable overrode the old "human maintainer only" line), no `- OPEN` ledger line retires here, and `--fix-only` stays undocumented until its code lands.
...
- [Units 84+85 audit PASS: the fixed-facts adapter half lands](gates-and-board.md#units-8485-audit-architect-2026-07-14-pass--every-gate-re-run-and-reproduced-on-this-mac-the-verbatim-move-claim-proved-at-the-ast-level-an-independent-unscripted-mutation-probe-of-the-horizontal-law-both-deviations-approved-the-v2-fixture-defect-class-routed-as-one-repair-unit-committed-on-the-branch) — every gate re-run and reproduced (2026-07-14); verbatim resolver move proved at the AST level; my own probe gutted the horizontal law and three family gates redded it; both deviations approved (shared refusal sites in inference.py; the new cs-adapter-identity gate); the geo-paths + four-smoke schema-v2 fixture class routed as ONE rider repair (facts_yaml AND support= in the same touch, 0161); geo-paths joins the standing torch floor list; committed on claude/amazing-keller-e798b6, unit-41 witness delta left for its own audit.
