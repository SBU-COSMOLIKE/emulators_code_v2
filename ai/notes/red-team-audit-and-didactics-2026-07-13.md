...
PYTHONPATH=. python3 ai/gates/checks/board_selftest.py
python3 ai/gates/checks/generator_seed.py
```
...
python3 -m py_compile emulator/training.py emulator/experiment.py \
  ai/gates/run_board.py ai/gates/checks/board_selftest.py \
  ai/gates/checks/cmb_identity.py ai/gates/checks/finite_contract.py
```
...
`ai/gates/board_config.json` is a tracked file below `ai/gates/`, so Git includes it
in that pathspec.  The comments at `ai/gates/run_board.py:716-727`, the config
dump text near line 1242, and `ai/notes/gates-and-board.md` all say it is
...
- construct the watched pathspec with an explicit exclusion for exactly
  `ai/gates/board_config.json`; do not exclude the rest of `ai/gates/`;
...
- in the same test, modify `ai/gates/board.py` and prove the decision turns red,
...
4. Do not use an internal audit-code prefix in an assertion identifier.
   ...
   internal project codes remain in `ai/notes/`.
...
`ai/gates/run_board.py::_resume_state` checks the stored code and input digests,
...
- `_gate_code_digest` hashes `inspect.getsource(gate.run)` and literal
  `ai/gates/checks/*.py` files mentioned in that function.  It omits shared
  helpers in `ai/gates/board.py`, runner behavior in `run_board.py`, imported
...
`emulator/`, `ai/gates/`, and root Python drivers, but not
...
- `ai/gates/README.md` and `emulator/README.md` still call the board a 32-test
  board.  The registry has 40 gates.
- `ai/gates/README.md` says unknown selectors warn and proceed.  The landed
...
The Fable Architect's broader untruncated scan of `emulator/` and `ai/gates/`
...
public methods/classes without docstrings in `emulator/`, 125 in `ai/gates/`,
...
6. `ai/gates/board.py`, `ai/gates/run_board.py`, and `ai/gates/checks/*.py`: every
...
`ai/gates/checks/finite_contract.py` contains a long module chronology but gives
several 80--150 line `check_*` functions one sentence, while
`ai/gates/checks/cmb_identity.py` mixes physical claims, fixtures, retired rules,
...
1. Submit preflight finding `RT-IMPL-02` to the Fable Architect: the executed
   ...
   exclude exactly
   `ai/gates/board_config.json` from the dirty-tree pathspec and prove both the
...
`ai/gates/board_config.json` is the first line, so `_dirty_lines` cannot recognize
...
`ai/gates/checks/logscan.py::byte_identity` returns true when both selected-line
lists are empty.  `ai/gates/board.py::_golden_leg` also discards both subprocess
...
`ai/gates/checks/mps_smoke.py` filters zero rows before applying its positivity
...
`ai/gates/board.py` says the production-diagnostic harness confirms a PDF, but
...
`ai/gates/checks/diagnostics_domain.py` joins two forbidden-pattern absences with
...
`ai/gates/checks/ge_c_eval_bs.py` computes a separate gate-side array of per-row
...
`ai/gates/checks/stage_ram.py` calls a two-column target fixture one-wide.
...
In `ai/gates/board.py`, `head-scheduler-override` does not parse an LR cut;
...
`ai/gates/checks/gsv_bitwise_drift.py` advertises drift tests for both activation
...
`ai/gates/checks/gt_b_triangle.py` reduces the figure to positive global counts
...
`ai/gates/checks/gct_parity.py` derives `masked` from the same rebuilt geometry
...
`emulator/README.md`, `ai/gates/README.md`, and `syren/README.md`) in separate
...
executable evidence in `ai/gates/README.md`. The package README then needs one
...
`../ai/notes/<file>.md#<stable-anchor>`, and a link check must cover all READMEs.

### DIDACTICS-87 -- the gate README teaches obsolete scope, resume, and log semantics

`ai/gates/README.md:3-7` calls the harness a cosmic-shear suite that runs “every”
...
`logs/<test>.log` (`ai/gates/README.md:46,147`), while
...
`ai/gates/README.md:101-108` admits its central table is incomplete. The current
...
`ai/gates/README.md:66-72` tells every user to edit `board_config.json`; its own
...
(`cobaya_theory/emul_baosn.py:333`, `ai/gates/checks/bsn_smoke.py:374`). Use
...
- `ai/gates/README.md` legitimately explains evidence, but it must explain the
...
claimed only `ai/gates/checks/gt_b_triangle.py`, as permitted by the Wave-3
...
`ai/gates/`. Those files, along with `board.py` and `run_board.py`, were expressly
...
README diary language, ai/gates/check teaching after D4+D5, and lane-3 protocol
...
- `python3 -m compileall -q emulator cobaya_theory syren
  compute_data_vectors ai/gates/checks` completed successfully;
- `PYTHONPATH=. python3 ai/gates/checks/board_selftest.py` completed with its
  all-pass verdict;
...
in `ai/notes/`.  The policy discussion at 738-751 and landing narration at
...
```text
Cocoa/.local/bin/python ai/gates/checks/scalar_identity.py
Cocoa/.local/bin/python ai/gates/checks/finetune_identity.py
Cocoa/.local/bin/python ai/gates/checks/transfer_identity.py
Cocoa/.local/bin/python ai/gates/checks/finite_contract.py
```
...
`ai/gates/checks/mps_identity.py` repair to the Red Team. The producer remains
...
```text
PYTHONPATH=. MPLBACKEND=Agg Cocoa/.local/bin/python ai/gates/checks/mps_identity.py
```
...
/Users/vivianmiranda/data/COCOA/june2026/cocoa/Cocoa/.local/bin/python
ai/gates/checks/scalar_smoke.py` returns zero and ends
...
`ai/gates/checks/mps_identity.py` is under the separate `25M-36` repair while
this claim starts.  This branch will therefore place the pure CPU mutation
legs in `ai/tests/test_grid2d_const_mask.py` and record the exact existing-gate
...
`ai/tests/test_grid2d_const_mask.py`, and records the readback in
`ai/notes/families-background-mps.md`. The focused command
...
test fixtures. The complete `ai/tests/` discovery run is 11/11 green.
...
also pass. Only `ai/gates/checks/bsn_identity.py`, the BSN home note, and this
...
python3 -m py_compile emulator/designs/blocks.py emulator/designs/plain.py \
  emulator/designs/ia.py ai/tests/test_trf_token_width.py
PYTHONPATH=. /Users/vivianmiranda/data/COCOA/june2026/cocoa/Cocoa/.local/bin/python \
  ai/tests/test_trf_token_width.py
# Ran 5 tests ... OK
PYTHONPATH=. /Users/vivianmiranda/data/COCOA/june2026/cocoa/Cocoa/.local/bin/python \
  -O ai/tests/test_trf_token_width.py
# Ran 5 tests ... OK
PYTHONPATH=. /Users/vivianmiranda/data/COCOA/june2026/cocoa/Cocoa/.local/bin/python \
  -m unittest discover -s tests -v
# Ran 22 tests ... OK
PYTHONPATH=. /Users/vivianmiranda/data/COCOA/june2026/cocoa/Cocoa/.local/bin/python \
  ai/gates/checks/cmb_identity.py
...
`ai/gates/checks/gb_c_berhu_reduce.py` still called the method unbound with
...
is `ai/gates/checks/gb_c_berhu_reduce.py`; the home-note readback is in
`ai/notes/training-stack.md` under the berHu evidence block. `ai/gates/board.py`,
`ai/gates/run_board.py` and `emulator/losses/core.py` remain byte-identical. This
...
The CPU regression is a dedicated `ai/gates/checks/generator_ranges.py` child.
...
are retained in `ai/notes/data-generation-and-cuts.md` under "25M-38
...
`ai/gates/checks/logscan.py`, `ai/gates/checks/stage_ram.py` and
`ai/gates/checks/board_selftest.py`. The first file owns the numerical predicate.
The second owns the inaccurate two-column fixture description. The third owns
the five required pure-Python controls. `ai/gates/board.py` and
`ai/gates/run_board.py` remain with the Implementer and are outside this visit.
...
The Cocoa Python run of `ai/gates/checks/stage_ram.py` ends
...
landing history remain in `ai/notes/`.
...
Team under the ruling in `ai/notes/gates-and-board.md`, commit `f2f448c`. The
candidate branch is `codex/finite-contract-harness`, based on main `5456133`.
The executable diff owns only `ai/gates/checks/finite_contract.py`. Production
code, `ai/gates/board.py`, the runner and `texnotes/` are unchanged. The detailed
...
`ai/gates/run_board.py --list` returns 0. The full board self-test returns 0 and
...
functional scope is `ai/gates/checks/scalar_smoke.py` only.  The required durable
resume is also recorded in `ai/notes/gates-and-board.md`.  The board declaration
is reserved to the Implementer and `ai/gates/board.py` is byte-identical.
...
`ai/gates/board.py`, `texnotes/` and `ai/tools/` were untouched.  The only executable
additions are:

- `ai/gates/checks/redteam_unit41_policy_witness.py`; and
- `ai/gates/checks/redteam_unit53_manifest_witness.py`.
...
```text
$ PYTHONPATH=. /Users/vivianmiranda/data/COCOA/june2026/cocoa/Cocoa/.local/bin/python ai/gates/checks/redteam_unit41_policy_witness.py
...
$ PYTHONPATH=. /Users/vivianmiranda/data/COCOA/june2026/cocoa/Cocoa/.local/bin/python ai/gates/checks/redteam_unit53_manifest_witness.py
...
to `ai/notes/conventions-and-workflow.md` was neither edited nor included.
...
cp "$SOURCE/ai/gates/checks/redteam_unit41_policy_witness.py" \
  "$TARGET/ai/gates/checks/"
cp "$SOURCE/ai/gates/checks/redteam_unit53_manifest_witness.py" \
  "$TARGET/ai/gates/checks/"
git -C "$SOURCE" diff -- \
  ai/notes/red-team-audit-and-didactics-2026-07-13.md \
  ai/notes/training-stack.md ai/notes/MEMORY.md | git -C "$TARGET" apply -
git -C "$TARGET" add ai/gates/checks/redteam_unit41_policy_witness.py \
  ai/gates/checks/redteam_unit53_manifest_witness.py \
  ai/notes/red-team-audit-and-didactics-2026-07-13.md \
  ai/notes/training-stack.md ai/notes/MEMORY.md
...
eight daemon defects, three repro scripts under `ai/tests/`, and one
...
4. None of the three repro scripts exists under the reachable `ai/tests/`
...
daemon (copied to `.scratch-tools-audit/ai/tools/mailbox_daemon.py` so the
...
Router, `ai/tools/handoff_router.py` (all six claims real):
...
Daemon, `ai/tools/mailbox_daemon.py` (beyond executed probes A/B/C):
...
- R4. Re-request the audit as a one-line delta ("codex/tools-review
  ...
  `main...96e5f26` scoped to `ai/tools/` + `ai/tests/` + the register;
...
5. The reachable copy of this register still carries TEX-PROSE-07/08 as OPEN
   ...
   are visible as `- OPEN` lines on `ai/notes/backlog.md` (the unit-94
...
- R4. On publication, the audit re-request is a one-line delta
  ...
  fixtures and named `UNAVAILABLE` legs in `ai/gates/board.py`; the hash and
...
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0118-to-fable.md
...
4. `ai/notes/backlog.md:22` carries the unit as `- OPEN` (the unit-94
...
1. **Phantom source-of-record.** Dispatch `0102-to-sol` names
   "ai/notes/training-stack.md, the unit 96 section (add-or-toggle vs
   declared unmasked artifact); the contract lives there."  It does not:
   `git show 204748e:ai/notes/training-stack.md` contains ZERO occurrences of
...
2. **Kept-core authority tension (FLAG for the user, decides at the fetch
   step).**  `ai/notes/gates-and-board.md:5587` rules: "NOT pre-authorized
...
- `ai/gates/checks/unit96_const_mask_authenticity.py` does not exist at
  `204748e` — the witness is a new deliverable, as claimed.
- `ai/tests/test_grid2d_const_mask.py` exists at `204748e`, and I re-ran it
...
nothing here accuses the work.  `ai/notes/backlog.md:22` stays OPEN.  Repair
...
1. Scope diff `204748e..22f425d4`: touched files must be exactly
   `emulator/results.py`, `ai/gates/checks/unit96_const_mask_authenticity.py`,
   and the three notes files named in the handback.  Any other touched
   path — `ai/gates/board.py`, existing checks, `ai/tests/`, thresholds,
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0122-to-sol.md
protected TeX tip:   codex/tex-prose-04-06 at 5546a0fd74d9536fdab42bfc8352411fb144752d
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0123-to-sol.md
...
ai/notes/red-team-audit-and-didactics-2026-07-13.md and Unit 94 implementation
readback: uniform support resolves in interval coordinates in
ai/notes/data-generation-and-cuts.md, both inside the source clone.  Commit
...
- ai/gates/checks/redteam_unit94_boundary_witness.py: the independent witness
...
sidecar/schema/shared reader, ai/gates/board.py, any shared check, or texnotes/.
...
- PYTHONPATH=. python3 ai/gates/checks/board_selftest.py: rc 0, final
...
Landing block, printed only:
    ...
    record branch:       claude/amazing-keller-e798b6
    record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
    outbound transport:  ai/notes/mailbox/0125-to-fable.md
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0126-to-fable.md
protected TeX tip:   codex/tex-prose-04-06 at 5546a0fd74d9536fdab42bfc8352411fb144752d
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0128-to-sol.md
protected TeX tip:   codex/tex-prose-04-06 at 5546a0fd74d9536fdab42bfc8352411fb144752d
TeX branch action:   none — publication remains the user-side fetch above
main action:         none — no merge or push is authorized
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0127-to-fable.md
protected tips:      codex/tools-review at 96e5f26a778f759b665292c1bb35c74ee17daf3c
                     codex/tex-prose-07-08 at f085260ee0df3097fa1438dbaff72251d0ef2205
...
4. `ai/notes/backlog.md:21` still carries the unit `- OPEN` with the
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0129-to-sol.md
...
`ai/gates/checks/redteam_unit94_boundary_witness.py` + the two records;
...
2. Exact-base board self-test, INDEPENDENTLY re-run: the full 204748e tree
   ...
   ai/gates/checks/board_selftest.py` run there: rc 0, **176 [PASS] / 0
...
5. Unlanded-candidate confirmation: `nextafter`,
   ...
   these greps ran from `ai/notes/mailbox/` and returned false negatives;
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0131-to-sol.md
...
the Architect-side instance of the same discipline.)  `ai/notes/MEMORY.md`
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0135-to-user.md
protected tip:       codex/unit94-boundary-interior at a0a03a9f06541eaa8dfbbb4968f53dacfe9d4849
clone action:        none — the exact tip stays frozen
...
`git show HEAD:ai/notes/red-team-audit-and-didactics-2026-07-13.md` carries
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0137-to-user.md
protected TeX tip:   codex/tex-prose-04-06 at 5546a0fd74d9536fdab42bfc8352411fb144752d
clone action:        none — the exact tip stays frozen
branch publication:  user runs the consolidated four-fetch block in the 0121 entry
main action:         none — no merge or push is authorized
...
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md
outbound transport:  ai/notes/mailbox/0138-to-user.md
protected tips:      codex/tools-review at 96e5f26a778f759b665292c1bb35c74ee17daf3c
                     codex/tex-prose-07-08 at f085260ee0df3097fa1438dbaff72251d0ef2205
...
`ai/gates/checks/redteam_unit94_boundary_witness.py` (+1010), and the two
...
`ai/gates/checks/redteam_unit94_boundary_witness.py` does not exist — the
...
```text
record branch:       claude/amazing-keller-e798b6
record file:         ai/notes/red-team-audit-and-didactics-2026-07-13.md (this entry)
...
ai/gates/board.py --self-test                rc 0
git diff --check                          rc 0
```
