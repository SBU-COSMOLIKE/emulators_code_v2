...
- files: temporary covariance and parameter-name sidecars plus synthetic
  ...
  temporary directory.
- subprocess: `ai/gates/checks/scalar_identity.py`; no nested subprocess (the
...
in `ai/gates/` exercising them, so a regression would have been silent.
...
- subprocess: `ai/gates/checks/scalar_smoke.py`; that child additionally runs
...
- subprocess: `ai/gates/checks/cmb_identity.py`; no nested subprocess, real
...
- subprocess: `ai/gates/checks/cmb_smoke.py`, which launches
...
- `ai/gates/checks/cmb_identity.py`: three eq-6 known-answer legs
  ...
  untouched.
- `ai/gates/checks/cmb_smoke.py`: leg 2b (check_cov_nondiagonal) gains an
...
- `ai/gates/board.py`: the cmb-identity + cmb-smoke docstrings, the
...
FROZEN for this delta; only ai/gates/checks/cmb_identity.py changes.
...
`python ai/gates/run_board.py --force-rerun cmb-identity cmb-smoke
...
**Scope kept:** ai/gates/checks/cmb_identity.py ONLY (the sole uncommitted
...
**Close (user-run, workstation):** `python ai/gates/run_board.py
...
- The converge_rtol = 1.0 fixture choice for the width-3 leg is
  ...
  touched (frozen).
- One stale line recorded, NOT a blocker: ai/gates/board.py's cmb-identity
  ...
  next unit that touches ai/gates/.
...
`python ai/gates/run_board.py --force-rerun cmb-identity transfer-identity`
...
6. README prose in plain language; ledger codes stay in ai/notes/.
...
Files (queue 42): emulator/losses/cmb.py, ai/gates/checks/cmb_identity.py,
ai/gates/board.py, ai/notes/families-scalar-cmb.md.
...
4. The caller rule applies uniformly across the CMB / grid / grid2d
   ...
   emulator/diagnostics.py + ai/gates/checks/ is part of the landing.
...
(ai/gates/checks/cmb_smoke.py) passes params_whitened in the needs_params branch
...
- Five new covariance-owned pure witnesses under `ai/gates/checks/`, one for
...
```text
$ python3 ai/gates/checks/redteam_covariance_schema_witness.py
PASS: covariance schema rejects every malformed input
$ python3 ai/gates/checks/redteam_covariance_stencil_witness.py
PASS: covariance stencil rejects no-op perturbations
$ python3 ai/gates/checks/redteam_covariance_psd_witness.py
PASS: every covariance PSD witness discriminated
$ python3 ai/gates/checks/redteam_covariance_finite_witness.py
PASS: all covariance finite-arithmetic witness legs passed
$ python3 ai/gates/checks/redteam_covariance_params_witness.py
...
$ python3 -m py_compile compute_data_vectors/compute_cmb_covariance.py \
    ai/gates/checks/redteam_covariance_schema_witness.py \
    ai/gates/checks/redteam_covariance_stencil_witness.py \
    ai/gates/checks/redteam_covariance_psd_witness.py \
    ai/gates/checks/redteam_covariance_finite_witness.py \
...
Traceback (most recent call last):
  File "ai/gates/checks/cmb_identity.py", line 89, in <module>
    import torch
...
- `PYTHONPATH=. python3 ai/gates/checks/board_selftest.py` -> ALL PASS at the
  tip.
...
no shared ai/gates/checks file, no board.py, no threshold, no fixture of any
...
in-flight fixed-facts adapter half (which edits cobaya_theory/*, ai/gates/*,
...
Commit note for this record: ai/notes/families-scalar-cmb.md currently also
