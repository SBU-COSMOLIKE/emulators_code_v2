...
- Convention: `45M-xx` codes appear only as commit-message provenance and in
  `ai/notes/`; per 45M-85 they are stripped from all Python prose.

Reproduce the CPU acceptance from the repo root:

```
cd gates && python3 run_board.py --list                  # rc 0 => evidence map validates
PYTHONPATH=.. python3 ai/gates/checks/board_selftest.py     # 33/33 ALL PASS
```
...
|---|---|---|---|
| 45M-85: strip all 84 internal `45M-*` codes from `emulator/` + `ai/gates/` Python prose | `2807d3f` | 14 files | 11/14 AST-identical, 3 differ only in a human-facing string |
...
| 45M-88: "How a gate teaches its evidence" + glossary terms | `bf62114` | `ai/gates/board.py` (module docstring) | mine |
