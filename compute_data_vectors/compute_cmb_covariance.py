...
def main():
  ...
  # refuse to overwrite: an emulator may already be trained against the
  # existing file, and silently replacing it would change what that
  # emulator's saved whitening meant.
  if os.path.exists(out_path):
    raise FileExistsError(
      "covariance output already exists: " + out_path + ". Choose a new "
      "--output name, or delete the old file deliberately and rerun.")
  # every persisted array must be finite: one nonfinite entry (for example
  # an overflowed beam exponential in the noise) would poison the training
  # whitening that reads this file.
  for key, value in out.items():
    if key == "provenance":
      continue                       # the provenance is a JSON string
    if not np.isfinite(np.asarray(value)).all():
      raise ValueError(
        "computed covariance array " + repr(key) + " contains a nonfinite "
        "value; nothing was written. Check the noise/beam settings against "
        "lmax (the beam exponential grows without bound in l).")
  # write to a temporary name first, then rename: a crash mid-write must
  # not leave a truncated file that a training run would trust.
  tmp_path = root + "/chains/" + args.output + ".tmp.npz"
