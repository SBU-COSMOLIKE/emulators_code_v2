...
def make_scalar_chi2(geom):
  ...
  The scalar analogue of make_chi2: there is no rescaling / ia / pce
  variant (those are data-vector concepts), so this simply wraps the
  ScalarGeometry in a ScalarChi2. Kept as a factory so the driver and
  experiment build the loss by a call, mirroring make_chi2.
