...
# file the generator wrote into the project chains/ folder (grid sidecars,
# syren base dumps, the CMB covariance from compute_cmb_covariance.py).
...
def resolve_cocoa_config(args):
  ...
  and rewrites every data-block file path to an absolute path under the
  project's chains/ folder: the flat keys (train / val dv, params,
  covmat, failure masks) and the family sub-block files (grid/grid2d
  axis sidecars, syren base dumps, the CMB covariance). The YAML lists
  bare filenames; an already-absolute path passes through unchanged.
  Resolving here, not in the YAML, lets the driver run from $ROOTDIR
  (the cocoa launch directory) without a cwd-relative path breaking.
