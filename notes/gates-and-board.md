...
| NEXT | the two-red rerun | merge the pending branch commit(s), paste gates/logs/transfer-identity.log for the open red, then on the workstation: `python gates/run_board.py --force-rerun cmb-smoke transfer-identity` (~10 min, cmb-smoke dominates; its eq-6 red now self-diagnoses via the stdout tail if anything else surfaces). Then: the five artifacts, EMUL2, the dense-covariance audit |
