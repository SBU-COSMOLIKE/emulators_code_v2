...
| NEXT | re-run after pull | bsn-smoke with the logposterior lifecycle + mps-smoke (D-MP9, was still running); full green = GEO's acceptance + the baseline for D-CM12; then the user's full regression pass via `--force-rerun-all` |
