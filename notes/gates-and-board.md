...
| NEXT | re-run after pull | mps-smoke with the law-agnostic pin (~12 min); full green = the first 32/32 = GEO's acceptance + the baseline for D-CM12; then the user's full regression pass via `--force-rerun-all` |
