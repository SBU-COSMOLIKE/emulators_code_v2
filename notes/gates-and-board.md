...
| NEXT | the D-CM11-A unit + one rerun | the Implementer lands the eq-6 normalization fix (spec: families-scalar-cmb.md "D-CM11-A"; new oracle legs in cmb-identity), the user pastes gates/logs/transfer-identity.log for the open red, then one workstation pass: `python gates/run_board.py --force-rerun cmb-identity cmb-smoke transfer-identity` (~12 min, cmb-smoke dominates; covariance failures now self-diagnose via the stdout tail). Then: the five artifacts, EMUL2, the dense-covariance audit (producing side blocked on D-CM11-A) |
