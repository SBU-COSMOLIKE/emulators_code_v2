...
| NEXT | fixture delta + one rerun | The eq-6 normalization implementation is merged at HEAD 7f455e6 and independently accepted on the Mac math scope (maximum relative error about 8.1e-14). The Implementer updates the fixture so raw lens-potential power and CAMB's scaled array are distinguishable; the user pastes gates/logs/transfer-identity.log for the open red, then runs `python gates/run_board.py --force-rerun cmb-identity cmb-smoke transfer-identity` (~12 min, cmb-smoke dominates; covariance failures self-diagnose via the stdout tail). Then: the five artifacts, EMUL2, and the dense-covariance audit. |
