...
| NEXT | the post-merge pass | merge the seven pending branch commits, then re-run the three fixed reds and force the gates whose legs are new on the merged tip (ONE --force-rerun flag, ids space-separated — nargs="+"): `python gates/run_board.py --force-rerun ema-off-identity bsn-identity mps-identity scalar-identity cmb-identity transfer-identity cmb-smoke` (the three reds rerun + the identity gates first-execute check_npce / check_diagonal / the phase legs; cmb-smoke first-executes eq-6 leg 2b; ~15 min, cmb-smoke dominates). Then: the five artifacts, EMUL2, the D-CM12 audit |
