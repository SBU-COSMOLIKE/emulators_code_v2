...
| NEXT | re-run after pull | mps-smoke past camb_truth (the 5% CAMB comparison + the range guard, first execution); full green = the first 32/32 = GEO's acceptance + the baseline for D-CM12; then the user's full regression pass via `--force-rerun-all` |
