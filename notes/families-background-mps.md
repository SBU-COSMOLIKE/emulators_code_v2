...
## UNIT 69 DONE (Opus, 2026-07-13): both MPS getters default nonlinear=True; mps-identity carries the protocol-guard + discrimination legs

Code fix (cobaya_theory/emul_mps.py): get_Pk_grid and get_Pk_interpolator
now default nonlinear=True -- Cobaya's BoltzmannBase public default -- so a
likelihood that omits the argument receives the NONLINEAR spectrum the
protocol promises, not the linear one. Both explicit branches are byte-
identical to before (the state dict already stores both the (Pk_grid, True)
nonlinear and (Pk_grid, False) linear keys; only the default value moved).
Both docstrings now name the omitted-call behavior: nonlinear is the public
default; a caller requests the linear spectrum explicitly.

Gate legs (gates/checks/mps_identity.py check_adapter, on the stubbed
adapter's real computed state): (1) get_Pk_grid() omitted == explicit
nonlinear=True and != nonlinear=False; (2) the same three-arm comparison
through get_Pk_interpolator at a stored node AND one interior between-node
point; (3) a sentinel state with deliberately separated linear=1 /
nonlinear=100 values proves catch power independent of the assembly; (4) a
mutation control proving the two branches genuinely differ, so the default
value decides the served spectrum (a revert to nonlinear=False reds legs 1-3
and the guard below); (5) the protocol guard -- inspect.signature pins BOTH
getters' nonlinear default against the INSTALLED BoltzmannBase (captured at
check-module load, before any cobaya.log stub), so an upstream drift OR a
local revert reds for review. board.py mps-identity maps= names the new legs.

Verification (Mac, cocoa-torch, cobaya 3.6.2): all five unit-69 legs PASS;
`get_Pk_grid + interpolator node round-trip` still PASS. One PRE-EXISTING
red is untouched and unrelated -- `bounded staging: streamed mean equals the
known answer` fails identically on the clean tree (stash-verified), a
workstation-sensitive staging leg, not introduced here.

Remaining (contract leg 5, workstation-owed): one real-Cobaya
provider-routed omitted-default call belongs on mps-smoke's existing
cobaya-vs-CAMB path (needs CAMB, so it certifies on the box, queue 5). The
direct-method + signature-pin legs already prove the fix on the Mac; the
provider leg is the belt-and-suspenders that only the workstation can run.

Sequencing note: the Architect's b306128 pairs unit 69 with unit 70 as one
increment. The files are disjoint (69 = emul_mps getters; 70 = cmb
diagnostics/losses), so 69 is committed on its own for early audit; 70 is
the pending paired half. The merge can wait for 70 if the Architect prefers
the single-increment landing.
