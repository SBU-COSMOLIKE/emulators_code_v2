...
### Current paths and the final layout

The examples in this guide use the paths that exist **now**. After every
earlier ledger unit closes, one physical-last migration changes them together:

| Current path | Final path |
| --- | --- |
| `notes/README.md` | `ai/README.md` |
| Remaining `notes/` records | `ai/notes/` |
| `tests/` | `ai/tests/` |
| `gates/` | `ai/gates/` |
| `tools/` | `ai/tools/` |

Until that migration lands, copy commands from the current-path column. The
final unit updates every command and link atomically; it leaves no compatibility
copies, wrappers, or duplicate README.
...
```bash
python3 tools/mailbox_daemon.py --dry-run
```
...
```bash
python3 tools/mailbox_daemon.py --watch \
...
```bash
python3 tools/mailbox_daemon.py --send fable \
  --unit "You are the Architect. Coordinate the version-flag unit in notes/version-flag.md."
```
...
```bash
python3 tools/handoff_router.py --status
```
...
- `notes/mailbox/done/` holds the consumed routing message.
- `notes/relay/` holds the raw dispatch log.
...
| Surface | Purpose |
| --- | --- |
| `notes/` | Blueprints, findings, verdicts, repair deltas, and the backlog |
| `notes/mailbox/` | Numbered routing messages and their lifecycle archives |
| `notes/relay/` | Raw dispatch logs |
| `tests/` | Reproductions and regression checks |
| `gates/` | The validation board and machine-run acceptance checks |
| `tools/` | Mailbox, status, and other AI-development utilities |
...
python3 gates/run_board.py --check
python3 gates/run_board.py --dry-run
```
...
On a configured workstation, run `python3 gates/run_board.py` or select a
...
```bash
python3 tests/tools_mailbox_daemon_role_models_repro.py
python3 tests/tools_mailbox_daemon_rendezvous_repro.py
```
...
```bash
python3 tools/mailbox_daemon.py --dry-run --send opus \
  --unit "You are the Implementer. Follow the ARCHITECT_HANDOFF in notes/version-flag.md."
```
...
```bash
python3 tools/mailbox_daemon.py --send opus \
  --unit "You are the Implementer. Follow the ARCHITECT_HANDOFF in notes/version-flag.md."
```
...
```bash
python3 tools/mailbox_daemon.py --send sol \
  --ticket-kind discovery \
  --unit "You are the Independent Red Team. Review the version-flag change named in notes/version-flag.md. Stay within that change."
```
...
python3 tools/mailbox_daemon.py --send sol \
  --ticket-kind closure \
  --unit "You are the Independent Red Team. Close the existing manifest item described in notes/backlog.md."
```
...
```bash
python3 tools/mailbox_daemon.py --ping opus
```
...
`notes/mailbox/.dispatch-history/` plus a verified move to `failed/`.
...
`notes/backlog.md`. Unsent ledger work still counts because it is still owed.
...
```bash
python3 tools/mailbox_daemon.py --watch --fix-only yes
```
...
Every checkout has its own `notes/mailbox/`. A valid send from the main
...
6. Run `python3 tools/mailbox_daemon.py --dry-run` there.
...
The copy of `tools/mailbox_daemon.py` you launch determines the mailbox,
...
```bash
python3 tools/mailbox_daemon.py --help
```
