...
The complete pipeline appears below. Each numbered box is a state of the run,
not a function call. Each arrow names the code boundary that must validate the
next state.
...
observable data vectors. $N$ is the total number of cosmologies, and $B$ is
the number processed in one batch. $P$ and $P_{\mathrm{enc}}$ are the raw and
encoded parameter widths. $D$ is the physical data-vector width, and $K$ is
...
owns the training state, and purple marks saved or served interfaces. The
...
using artifact schema version 2. Because the HDF5 record stores the resolved configuration,
...
parameters it needs straight from the `.h5`. Do not repeat the architecture
or parameter ordering in the sampling YAML. The older `ord` and `extrapar`
settings are not used. The whole theory block is:

```yaml
theory:
  emul_cosmic_shear:
    # Cobaya calls its external-class key python_path, not path.
    # This key selects the adapter from this repository.
...
three emulated CAMB products. `emul_mps` serves the matter power spectrum,
`emul_baosn` serves the expansion history, and `emul_scalars` serves the
sound horizon `r_drag`. Put all three theory blocks in one sampling YAML. That
...
How does accuracy improve as the training set grows? This driver retrains the
same model at several training-set sizes and plots `frac>0.2` against
...
Run this command under an external job timeout. Treat missing worker progress
as a failed run, and do not accept an output table from that invocation.
...
| Do not sweep a model class. | `model.name`, `model.ia` | Run one campaign per class and compare their output tables. |
| Refuse an unknown first path segment. | `modle.mlp.width` | A typo must not train the unchanged configuration repeatedly. |
...
table, and `<--out>.pdf` draws the same results as a curve.
...
The driver never divides one training across devices; each spawned worker owns
one complete training.

| Driver | Jobs | Split across GPUs | Extra flags |
|---|---|---|---|
| `sweep_ntrain` | one training per `N_train` | longest-processing-time scheduling; largest $N$ goes first to the least-loaded GPU | `--gpu-pack` |
...
runs several small trainings on the same card. A small model can leave the
card mostly idle because Python dispatch, rather than GPU computation,
dominates its wall time. Sharing the card can therefore finish the sweep
...
divides `--n-trials` across the workers. Reusing the journal resumes the study.
One GPU or Apple MPS runs the same study serially.

Do not judge a parallel study only by the best trial printed at the end.
Confirm that the worker logs are successful and that the number of newly
completed trials increased. A failed worker set can otherwise leave an older
completed trial as the reported best result.
...
**The non-cosmic-shear training-size wrappers currently require an explicit
`param_cuts` block.** Without it, the worker-count calculation can read the
missing `omegabh2_hi` key and stop before the first training point. Do not run
a scalar, CMB, background, or matter-power `sweep_ntrain` campaign from a
no-cut YAML.
...
| validation | `val_dv` | `val_params` | not used |
...
`n_train` and `n_val` are absolute row counts. They are not fractions. The
code applies the physical cuts first and then selects the requested number of
rows. If the rows that survive the cuts are too few, the run raises an error
instead of silently training on a smaller set.
...
| `berhu` | reversed Huber, defined below | equal in the bulk, rising in the window | push the bulk under the goal |
...
default is 10. These are chi-square values for one sample, not individual
residual coordinates. In the equivalent textbook BerHu notation, the
whitened-residual threshold is $\delta=\sqrt{k}$.
...
after the active mode, such as `berhu_capped:`. Do not provide both spellings;
precedence rule [D](#d-loss-block-spellings)
explains the resulting error.
...
The optimization stack has three blocks that work together.
...
**Weight decay acts only on true weight matrices.** AdamW's decoupled decay
adds a small pull toward zero beside the gradient step:

$$w \leftarrow w - \mathrm{lr} \lambda w$$

Here $\lambda$ is `weight_decay`. This update is separate from the gradient
update. Module role, rather than tensor shape, determines whether a parameter
receives it:
...
gradient. A few contaminated vectors therefore cannot dominate an update.
...
mean. The weight is detached from automatic differentiation. A sample
therefore cannot reduce its own weight instead of improving its prediction.
...
The optional exponential moving average, or EMA, is a running average of the
...
those templates. The IA amplitudes therefore never enter the network, and the
...
MLP stands for multilayer perceptron. It is the simplest network in this
...
slope is $\gamma$, and its right limiting slope is one. The orange dashed line
...
analytic signed-power transform has unit local slope at zero. The current
PyTorch expression instead returns a zero autograd derivative at exactly zero;
the plotted forward curve cannot display that backward-pass exception. These
curves illustrate the mechanisms. Learned parameters are feature-specific and
need not equal the plotted examples.

The vector PDF is generated by
[`texnotes/make_figures.py`](texnotes/make_figures.py). The PNG browser preview
is generated by
[`texnotes/render_readme_previews.py`](texnotes/render_readme_previews.py).
...
flowing through the layer. The `norm` subsection below draws this axis. The
...
|---|---|---|
| `H` | 2 | one bend; slope gamma to its left and slope 1 to its right |
| `power` | 3 | `H` plus a learned tail exponent: the tails may grow like $x^p$, with $p$ kept inside $[0.5, 1.5]$ |
| `multigate` | 3·`n_gates` + 1 | `n_gates` bends at learned positions, producing a slope schedule along x |
...
very large or very small. An activation responds strongly only over a limited
...
| `per_feature` | one pair per feature: 2·width, 256 at width 128 | each feature's individual operating point; use it when one shared pair cannot keep every feature inside its responsive window, and pair it with `tanh` |
...
cosmologies in the batch, and each column is one feature:
...
A feature is not a data-vector element, cosmological parameter, or sample.
The activation parameters gamma and beta use this same per-feature meaning.

Only the trunk ResBlocks read this key. The transformer head uses its own
internal LayerNorm, and the convolutional head has no normalization slot.
...
and its train/eval split stores running statistics in buffers outside the EMA
weight average. That separation risks baking one fixed mode into the compiled
evaluation copy. The library uses `affine` as the default replacement.
Choose `per_feature` when one shared scale and shift cannot keep every feature
inside its activation's responsive window.
...
neighborhoods. A **convolution** does exactly this: it slides a small set of
...
The data vector contains many tomographic bins laid end to end; it is not one
long curve. The head rearranges it into a grid with one row per bin, then runs
...
trunk output y                    in the trunk's rescaled units
   ...
   ▼                             data covariance; frozen and never
theta-order data vector          trained; converts into angular order
   │
...
their activations, and the gate. There are no adapter layers that consume
additional parameters. Because the last convolution starts at zero, the
correction is zero at epoch 1. The model therefore begins exactly equal to its
trunk.
...
shift per bin. The cosmology therefore chooses which bin corrections to
...
`n_mlp_blocks` layers, all at the token width. Separate weights for each bin
are a deliberate deviation from the textbook transformer,
...
them apart. A positional encoding performs that identification in many other
transformers, so this head does not need one. `shared_mlp: true` restores the
textbook sharing as an ablation.
...
and keeps the attention calculation away from saturation. It fills the role
of `model.norm` in the trunk and is not configurable here. Both branches end
in a zero-initialized layer, so every block is exactly the identity at
...
| `n_tokens` | CMB and BAOSN only: split the single spectrum into this many contiguous windows, which become the attention tokens. Each token has width `ceil(n / n_tokens)`, and `n_heads` must divide that width. Cosmic shear and matter power reject this setting because their tomographic bins or redshift slices already define the tokens. |
...
Each remains 26 × 26 for any allowed `n_heads`, so the parameter count and
matrix cost do not change. More heads create more, narrower attention tables
for each bin pair.
...
Single-phase models never break on a two-phase YAML. `resmlp` is the design
without a correction head. For this design, `train()` merges `trunk:` into the
top level and drops `head:`, `trunk_epochs`, and `freeze_trunk`. The same YAML
therefore drives both model families. On `resmlp`, the trunk block simply
becomes the global training configuration.
...
polynomials**, with one fixed curve for each degree. The curves themselves are
not learned. The fit chooses which curves enter and determines their
coefficients:
...
**orthogonal** on this interval: each curve carries information that the
others do not. This separation helps keep the least-squares fit stable as
terms are added. At evaluation time, a point just
...
cloud is first split into its main directions of variation by a singular
value decomposition, or SVD. This is the same basic idea as principal
components. Mode zero is close to an overall amplitude, and later modes
...
point, computed as if that point had been excluded from the fit. It is a
generalization test that needs no held-out data and can be evaluated without
refitting the model. It is
...
demonstrably poor base. Inspect the printed LOO value for every persisted
mode rather than assuming it passed `loo_max`. Modes rejected after an
accepted one are left to the refiner.
...
`p_max` is therefore the smoothness knob. The leave-one-out test, rather than
the term cap, decides the accepted size of each mode.
...
smooth low-order base. A multiplicative refiner has little leverage where the
...
form already gives the refiner separate leverage on every element. On a
...
replaces the chi-square loss construction. The block is also not a legal
sweep axis. It is a top-level block rather than a `train_args` leaf, so each
study fits one base. The collision rules are in the
[precedence appendix](#22-appendix-precedence-when-settings-collide).

The block works with every family: scalar, CMB spectra, background, and matter
power. These families use `form: residual`, and the base fits the same rescaled
...
`model:` block because the architecture is read from the saved artifact and
must not be restated. New parameter columns are handled automatically. For
example, a new covariance may add `w0` and `wa`. The existing weights remain
unchanged, and every new input starts with exactly zero influence. Use the
...
The saved emulator is the **base**, and all of its weights remain frozen. The
...
Scalar maps use [fine-tuning](#fine-tuning-train_argsfinetune) instead, so a
scalar `transfer:` block is an error.
...
multiplicative correction has no leverage there. These three families do not
yet support `transfer.refine`.
...
the extra accuracy. Joint refinement is currently available only for cosmic
...
sampled parameters rescaled as described in section 2
     ...
     │                          just narrower because a scalar map is small
...
Nothing is emulated between the two windows. Likelihoods must not query this
untrained redshift interval, and a query raises an error rather than silently
bridging the gap.
...
| `get_angular_diameter_distance` | angular-diameter distance, including the two-redshift variant |
| `get_luminosity_distance` | luminosity distance |

These are flat-universe formulas. The current guard rejects curvature only
when `omk` appears among an artifact's required input names; a fixed or global
nonzero `omk` can bypass it, and the quantity called `D_M` is radial $\chi$.
Use these artifacts only with `omk = 0`, and verify that condition outside the
adapter before inference.
Fine-tuning works separately for each artifact. The source and destination
must have the same quantity, grid, units, and law, and the `model:` block is
...
The ordinary training controls apply unchanged, including the correction
...
convolution channels and slides along $k$, mixing redshifts at similar
...
grid points carry no training signal. The geometry pins them instead of
trying to learn a constant column and reports the number pinned at startup.
...
The optional PCE block is suitable only for small research and smoke runs on
this family. It materializes the full thinned target on the GPU and performs a
dense float64 SVD on the CPU. The documented 50,000-row grid exceeds the
12 GiB test cards before SVD workspace. Do not use this PCE path for a
production MPS grid unless the full thinned target and the SVD workspace fit
in memory.
...
**Hybrid inference, or EMUL2,** uses CosmoLike's `use_emulator: 2` mode.
Instead of replacing the full data vector, it replaces three CAMB products in
one sampling YAML:
...
and nonlinear spectra. It uses a CAMB-compatible interpolator adapted from
CAMB by Antony Lewis, so a likelihood written against CAMB's provider does
not need to change.
...
**Do not request `sigma8` from this adapter.** Its current helper integrates
$k$ in 1/Mpc with a radius of 8 Mpc rather than $8/h$ Mpc. The returned value
is therefore $\sigma(8\,\mathrm{Mpc})$, not the conventional
$\sigma(8/h\,\mathrm{Mpc})$. Serving $P(k,z)$ is unaffected.

**The current adapter also has a Cobaya-registration limitation.** It
advertises products through Cobaya's input-support hook, and its Syren
requirement always asks for `As` even when an artifact uses `As_1e9`. The
example evaluate YAML defines both names, which hides this mismatch. Do not
use an EMUL2 result unless the actual Cobaya dependency resolver accepts the
...
|---|---|---|---|
| `dataset_generator_lensing.py` | cosmic shear; sections 2–13 | CosmoLike | one data-vector `.npy` plus parameter, covariance, and range sidecars |
...
machinery, and output contract below belong to the shared core and therefore
...
explore*. This region is broader than the posterior itself. A sampler that
walks to the edge of the posterior must still land inside the training
support, so the
...
parameters. Temperature widens the proposal. Correlation control fills
directions across the posterior degeneracy. The figure does not draw a
separate validation region. The rule is that validation must also remain
inside the trained support, so its score measures interpolation rather than
accidental extrapolation at the edge. The vector PDF and PNG preview use the
same two scripts linked below the opening pipeline figure.
...
**The machinery.** emcee uses `DEMove` for 90% of proposals and
...
checkpoint, and `--append` extends it. A failed physics evaluation is written
as zeros and marked in the failure file. Rerunning with `--loadchk 1` attempts
that row again.

**Check the failure file before training.** The generator can return exit
status zero while failed rows remain, and trainer staging does not read this
file. Require every failure flag to be zero. Otherwise, a zero-filled failed
row is treated as a physical target during training.

Use only `0 < --boundary < 1`. Other values are currently replaced by one
instead of being rejected.

**Protect checkpoint files before appending.** A checkpoint is a multi-file
set, and append writes its members sequentially. The resume census does not
yet include `.paramnames` or the background and matter-power axis sidecars. A
load exception can also fall through to fresh generation on the same output
roots. Keep an external copy before `--append`. If checkpoint loading prints
an exception, stop that invocation rather than allowing it to replace the
files.

**The output contract.** `<T>` is the temperature tag for a tempered run and
...
covariance. It is the quantity that the inference actually uses.

Two ideas run through the whole pipeline:

1. **Whitening** rotates and rescales both inputs and outputs. The network sees
   decorrelated directions with unit variance instead of raw correlated
   numbers with very different scales.
2. **The chi-square metric** judges training and evaluation in the natural
   geometry of the data covariance, rather than by raw error in each element.
...
**1. Stage the data with `data_staging.py`.** The training set pairs one physical
...
**2. Whiten the inputs with `geometries/parameter.py`.** Raw cosmological parameters
...
**3. Whiten the output and retain the metric with
`geometries/output.py`.** The analysis mask keeps only selected entries from
...
**4. Build the loss with `losses/core.py`.** `make_chi2` wraps the output
geometry in the chi-square metric. [Appendix 20](#20-appendix-the-chi2-metric-mahalanobis)
...
worst-fit points early in training, and `focus` gives more weight to the
points that remain difficult. Sections [7](#7-trim) and [8](#8-focus) define
these controls.
...
object is built once and stored inside the selected loss. It is not reread or
inherited. The **`needs_params` capability flag** tells the pipeline whether a
loss also needs cosmological parameters. A future parameter-aware loss sets
this flag; the pipeline does not branch on a concrete class name.

**5. Choose the model from `designs/plain.py` or `designs/ia.py`.**
...
templates, and the loss combines them in closed form. The IA amplitudes never
...
**6. Feed the GPU with `batching.py`.** The loaders select one of three regimes
according to available device memory. If the encoded set fits, it remains on
...
**7. Train with `training.py`.** `run_emulator` builds the model, optimizer,
and scheduler from their specification dictionaries. Each epoch applies the
configured robustness schedules, trains on all batches, and evaluates the
...
**Mahalanobis distance** is the distance between two points measured *in units of
...
- **Diagonal `C`:** its entries are variances σᵢ² and it has no correlations.
...
The CNN uses the diagonal-whitening variant named `DiagonalGeometry`. That
variant breaks the equality. The loss therefore always keeps the explicit
`Cinv` contraction rather than collapsing to a mean squared error.

**Mnemonic:** Mahalanobis is Euclidean distance *after whitening*. The units
are σ, and the correlations have been removed.
...
$H(x)$, and three learnable generalizations are available. Select the family
...
slope is $\gamma$, and its right slope is one. Unlike $\tanh$, the derivative
does not vanish merely because the input is far into a tail. $\gamma$ controls
...
**Multi-gate:** `multigate` selects `GatedActivation`. It replaces the single
...
**Bounded power tail:** `power` selects `PowerGatedActivation`. It keeps the
...
**Gated power:** `gated_power` selects `GatedPowerActivation`. It multiplies
...
which value the run actually uses. Every
...
- The bake-off and a `model.activation` sweep rewrite the shared slot per
  curve. A pinned head stays fixed, and the driver prints this warning once at
  startup.
- Spellings: on a two-phase model `head: activation:` is a second spelling of
  the pin. The canonical spelling is `model.cnn.activation` or
  `model.trf.activation`, which also works for single-phase YAMLs. Providing
  both spellings is an error, even when they agree. `trunk: activation:` is
...
| `activation` | head only; an alias for `model.<head>.activation` that is consumed when the model is built | Legal in `head:` because the head trains only in phase 2. Illegal in `trunk:` because the same trunk modules exist in both phases. |

Phase blocks are changes relative to the top level, not independent
containers. Construction settings are run-global. `head: activation:` is the
...
| both spellings present | an error; there is no silent winner |
...
| the top-level `pce:` block | not a `train_args` leaf, so it cannot be swept or searched. Each study owns one fitted base. A copied PCE key under `train_args` would not refit the base. |
...
| `rescale` | configured only by a driver flag; there is no YAML key |
...
| optimizer / scheduler class | fixed to `AdamW` and `ReduceLROnPlateau`; a phase scheduler changes settings but not the class |
...
same artifact directly from Python. Cobaya is not involved in this second
path:
...
their order, every rescaling, and the output definition. The only remaining
choice is `device`, which may be `cuda`, `mps`, or `cpu`.
...
| matter power | `{"z": ..., "k": ..., "<quantity>": surface}` | The surface has shape `(nz, nk)` and remains in the stored target-law space. For a Syren law it is $\log(P/P_{\rm Syren})$. `emul_mps`, rather than `EmulatorPredictor`, multiplies the Syren base back. |
...
classes. A version-2 replacement requires retraining. [Section
...
AI Usage: This library was developed with Claude Code assistance under the
`dev` folder. Prof. Miranda heavily influenced the code at every
